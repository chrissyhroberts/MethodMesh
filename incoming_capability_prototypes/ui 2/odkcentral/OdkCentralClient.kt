...
object OdkCentralClient {
    ...
    suspend fun deployAndAssign(
        ...
        val blockingValidationErrors = template.validationIssues.filter { it.severity == com.example.methodmesh.ui.odk.OdkValidationSeverity.Error }
        ...
        // Do not hide catalogue/list failures. A failed list request can otherwise look like
        // an empty project and turn a useful server error into a confusing duplicate-form error.
        val forms = listForms(server, token, projectId)
        ...
        // MethodMesh XLSForms are example/test forms. Central conversion/workflow warnings
        // (including deleted-form ID collisions and pyxform advisory warnings) must not
        // interrupt this rapid-test path. Central still rejects conversion errors when
        // ignoreWarnings=true, so genuine invalid syntax remains blocking.
        val xmlFormId = if (deployment == null) {
            ...
            if (discovered != null) {
                requestJson(
                    serverUrl = server,
                    method = "POST",
                    path = "/v1/projects/$projectId/forms/${encodePath(discovered.xmlFormId)}/draft?ignoreWarnings=true",
                    token = token,
                    body = bytes,
                ...
                val created = requestJson(
                    serverUrl = server,
                    method = "POST",
                    path = "/v1/projects/$projectId/forms?ignoreWarnings=true",
                    token = token,
                    body = bytes,
                    contentType = XLSX_MIME,
            ...
            if (deployment.localSha256 != digest) {
                requestJson(
                    serverUrl = server,
                    method = "POST",
                    path = "/v1/projects/$projectId/forms/${encodePath(deployment.xmlFormId)}/draft?ignoreWarnings=true",
                    token = token,
                    body = bytes,
                    contentType = XLSX_MIME,
    ...
    private fun request(
        ...
        if (status !in 200..299) {
            ...
            val apiCode = root?.optString("code")?.takeIf(String::isNotBlank)
            val detailsValue = root?.opt("details")
            val details = jsonDetailText(detailsValue)
            val message = root?.optString("message")
                ?.takeIf(String::isNotBlank)
                ?: firstUsefulError(detailsValue)
                ?: apiCode
                ?: "ODK Central request failed ($status)."
            throw OdkCentralApiException(
                statusCode = status,
                message = message,
                apiCode = apiCode,
                details = details,
    ...
    private fun jsonDetailText(value: Any?): String? = when (value) {
    ...
    private fun firstUsefulError(value: Any?): String? = when (value) {
        null, JSONObject.NULL -> null
        is String -> value.takeIf(String::isNotBlank)
        is JSONObject -> {
            listOf("message", "error", "detail", "warning").firstNotNullOfOrNull { key ->
                value.optString(key).takeIf(String::isNotBlank)
            } ?: value.keys().asSequence().mapNotNull { key -> firstUsefulError(value.opt(key)) }.firstOrNull()
        }
        is JSONArray -> (0 until value.length()).asSequence().mapNotNull { firstUsefulError(value.opt(it)) }.firstOrNull()
        else -> value.toString().takeIf(String::isNotBlank)
    }
