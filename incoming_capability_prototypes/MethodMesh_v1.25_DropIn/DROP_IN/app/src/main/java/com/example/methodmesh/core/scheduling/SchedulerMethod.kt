...
object As100SchedulerMethod : As100Method {
    ...
    override val id = ID
    override val ref = ArchitectureRef(ArchitectureId(ID), "Method", "Create a MethodMesh schedule")
    override val descriptor = MethodDescriptor(
        id = ArchitectureId(ID), methodType = MethodObjectType.Workflow, name = "Create schedule", version = VERSION,
        description = "Schedule an ODK form, web form, or future MethodMesh process.", outputs = SchedulerFields.outputs,
        graphOutputs = listOf("methodmesh.schedule"), parameters = mapOf("category" to "Operations", "status" to "Experimental")
    )
...
abstract class SchedulerTransferMethod(
    ...
    override val descriptor = MethodDescriptor(id = ArchitectureId(id), methodType = MethodObjectType.Workflow, name = name, version = "1.0.0", description = descriptionText, outputs = emptyList(), graphOutputs = emptyList(), parameters = mapOf("category" to "Operations", "status" to "Production"))
