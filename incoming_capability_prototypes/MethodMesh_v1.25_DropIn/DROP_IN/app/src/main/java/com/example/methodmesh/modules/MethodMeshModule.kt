...
object MethodMeshModuleRegistry {
    ...
    fun install(modules: List<MethodMeshModule>) {
        ...
        val canonicalMaturityLabels = MaturityStatus.values().map { it.label }.toSet()
        require(methods.all { method ->
            listOf("maturity", "status")
                .mapNotNull { key -> method.descriptor.parameters[key] }
                .all { it in canonicalMaturityLabels }
        }) { "Capability maturity/status metadata must be exactly Production, Development, or Experimental." }
