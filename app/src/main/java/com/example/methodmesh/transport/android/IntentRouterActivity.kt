...
class IntentRouterActivity : ComponentActivity() {
    ...
    private fun workflowIntentFor(incoming: Intent): Intent =
        Intent(incoming).apply {
            setClass(this@IntentRouterActivity, ExternalWorkflowActivity::class.java)
            if (browserLaunch) {
                // Browser task flags (especially NEW_TASK) must not cross the
                // start-for-result boundary. Preserve URI grant flags only.
                flags = incoming.flags and URI_GRANT_FLAGS
            }
            // Non-browser callers retain their complete existing Intent
            // contract, including action, data/type, categories, extras,
            // ClipData and flags.
        }
