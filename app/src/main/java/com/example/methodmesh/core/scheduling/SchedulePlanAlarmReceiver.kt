...
class SchedulePlanAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        manager.notify(occurrence.id.hashCode(), NotificationCompat.Builder(context, CHANNEL).setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle(title).setContentText(occurrence.actions.firstOrNull()?.message?.ifBlank { "Scheduled activity due" } ?: "Scheduled activity due").setContentIntent(pending).setAutoCancel(true).build())
    }

    companion object { const val ACTION = "com.example.methodmesh.SCHEDULE_PLAN_ALARM"; const val CHANNEL = "methodmesh_schedule_plans" }
}
