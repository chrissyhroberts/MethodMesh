...
class SchedulePlanAlarmReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        manager.notify(occurrence.id.hashCode(), NotificationCompat.Builder(context, CHANNEL).setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle(title).setContentText(occurrence.actions.firstOrNull()?.message?.ifBlank { "Scheduled activity due" } ?: "Scheduled activity due").setContentIntent(pending).addAction(0, "Done", done).addAction(0, "Snooze", snooze).setAutoCancel(true).build())
    }
