...
class ClockAssuranceService(
    ...
    fun observe(policy: ClockAssurancePolicy): ClockAssuranceObservation {
        ...
        val wallStatus = when {
            ...
            // A negative offset uses the rollback tolerance, not the forward tolerance.
            offset >= -policy.wallClockRollbackToleranceMillis &&
                offset <= policy.wallClockDivergenceToleranceMillis -> WallClockStatus.CONSISTENT
