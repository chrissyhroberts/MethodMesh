...
Return a compact guitar chord fingering using six-character low-E-to-high-E fret notation.

## Canonical capability and surfaces

This capability is defined once by `As100ChordGuideMethod`. The module dashboard (where applicable), direct native use, preset execution, protocol execution and ODK/XLSForm are projections over that same method contract. The dashboard is not the implementation boundary. The capability remains independently discoverable through `MusicModule.as100Methods()`, `capabilityScreens()` and `capabilitySettings()`.
...
The primary beef field is `music_chord_guide_result`. All fields below remain contractually available even when the compact native UI does not normally show them:

- `music_chord_guide_result`
- `music_chord_guide_instrument`
- `music_chord_guide_chord`
- `music_chord_guide_shape`
- `music_chord_guide_format`
- `music_chord_guide_status`
- `music_chord_guide_audit_json`
- `music_chord_guide_error`
- `methodmesh_full_json` — transport-level complete payload when requested by ODK/external transport.

`music_chord_guide_audit_json` is the module audit payload (`methodmesh.music.audit.v1`) and remains secondary to the main result.
...
The stable method ID `music.chord_guide` remains an independent protocol step and uses the same declared inputs/outputs. Schedule/widget launches do not create a separate implementation; closeout is delegated to the shared launch-origin contract.
