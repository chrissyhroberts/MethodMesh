...
```text
com.example.methodmesh.EXECUTE_METHOD(method_id='attestation.anchor_bundle',input_study_id='my_study',operator_id='operator_001',return_mode='flat')
```
...
outputs and MethodMesh never treats them as settings, so blank return
