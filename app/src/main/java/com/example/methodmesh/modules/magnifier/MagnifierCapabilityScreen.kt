...
object MagnifierCapabilityScreen : CapabilityScreenSpec {
    ...
    override val description = "Zoom in, hold focus if useful, freeze the frame, inspect it, then capture the image you want to return."
    ...
    override fun Render(
                ...
                object : ImageCapture.OnImageSavedCallback {
                    override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                        mainHandler.post {
                            ...
                            inspectionPanX = 0f
                            inspectionPanY = 0f
                            statusText = "Frame frozen. Inspect it, choose a filter, then use this image."
                        }
                    }
        ...
        val result: ExecutionResult? = remember(
            ...
            val values = linkedMapOf(
                ...
                MagnifierFields.TORCH_MODE to if (torchOn) "on" else "off",
        ...
        DisposableEffect(controller, lifecycleOwner, hasCameraPermission) {
            if (hasCameraPermission) {
        ...
        LaunchedEffect(controller, hasCameraPermission) {
            if (hasCameraPermission) {
                delay(250)
            ...
            if (frozenPath == null) {
                ...
                Row(
                    ...
                    if (allowTorch && torchAvailable) {
                        OutlinedButton(onClick = { setTorch(!torchOn) }, modifier = Modifier.weight(1f)) {
                            Text(if (torchOn) "Torch off" else "Torch on")
                        }
                    }
                    OutlinedButton(onClick = ::toggleFocusHold, modifier = Modifier.weight(1f)) {
                        ...
                        Row(
                            ...
                            filters.forEach { filter ->
                                FilterChip(
                                    ...
                                    onClick = { filterWireValue = filter.wireValue },
