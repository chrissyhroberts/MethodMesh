...
`odk_form_launcher` searches forms available locally in ODK Collect or Kobo Collect and opens a matching form for completion. The selector may be the form ID or display name. MethodMesh does not download forms; the form must already be present on the device.

## Android intent

Use the canonical MethodMesh action:

```text
com.example.methodmesh.EXECUTE_METHOD(method_id='odk_form_launcher',input_project_id='my_project_id',input_form_selector='my_form_id',return_mode='flat')
```

External invocation searches and opens the form immediately. When ODK returns, MethodMesh returns the matched form details and, when available, the saved instance URI. A dashboard invocation waits for the operator to press **Use result**.
...
The form must be downloaded and available in ODK Collect or Kobo Collect. MethodMesh uses the supported exported forms-provider interface and does not rely on private activity names. On the dashboard, save the project name and local project UUID to build a reusable project registry; then choose a saved project and one of its forms.
...
| `odk_launch_time_iso` | MethodMesh launch-result time. |
