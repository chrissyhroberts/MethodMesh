...
The provisioner scans for nearby MethodMesh sensor nodes, connects over BLE, reads the node manifest, writes a persistent configuration command, reads a confirmation sample, and saves the resulting device profile to the MethodMesh device registry.
