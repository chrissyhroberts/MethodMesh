...
- reuse of the existing MethodMesh Chance package as a shared randomness, dice, coin and probability subsystem;
...
- integration with the shared MethodMesh Chance Engine;
...
- consumption of shared probability, sampling and repeated-trial functions from the MethodMesh Chance Engine where suitable;
...
### 9.8 Winnable but not inevitable
...
## 11. Player model and character sheet
...
## 30. Character traits and achievements
...
## 31. GameDeck as a learning system
...
## 33. Preset generation from player profile
...
1. dice and coin behaviour can be invoked through the shared Chance Engine interface;
2. live randomness and deterministic seeded randomness are distinct modes;
3. deterministic mode reproduces the same result sequence for the same seed and engine version;
4. at least one GameLab simulation consumes shared Chance Engine probability/randomness services;
5. a game that needs ordinary dice/coin functionality does not require a second implementation.
...
The existing MethodMesh Chance package is the first concrete shared subsystem in this architecture. Its dice, coin, probability, sampling and related visual capabilities should be reused and exposed through a stable Chance Engine interface so that GameDeck and GameLab share one probability substrate rather than accumulating parallel implementations.
...
### 2.8 Reuse the existing MethodMesh Chance Engine

The existing MethodMesh `chance` package is an architectural dependency and SHOULD be reused rather than reimplemented.

Where its semantics are suitable, GameDeck and GameLab MUST reuse the shared MethodMesh Chance components for:
...
- roll and outcome histories;
- existing MethodMesh chance-related visual components.

Individual game modules MUST NOT independently implement equivalent chance or probability machinery without a documented technical reason.

The Chance Engine must support two distinct operating modes:

1. **runtime randomness** — unpredictable outcomes for live play;
2. **seeded randomness** — deterministic, reproducible pseudo-randomness for GameLab simulation, generated scenarios, replay and testing.

Game modules should consume chance services through a stable shared interface rather than reaching directly into implementation-specific internals.

The existing `chance` package is to be treated as source material for reuse and integration. Exact internal APIs are not assumed by this specification until the package has been inspected and mapped.
