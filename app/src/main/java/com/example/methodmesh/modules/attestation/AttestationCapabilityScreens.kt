    ...
    override fun Render(
            ...
            when (method) {
                AttestationVerificationMethod.Fingerprint -> {
                    BiometricAuthHelper.authenticate(
                        ...
                        description = "The biometric itself is not stored. MethodMesh records only that Android accepted local biometric authentication.",
                        cancelText = "Cancel",
                ...
                AttestationVerificationMethod.Pin -> {
                    BiometricAuthHelper.authenticateDeviceCredential(
                        ...
                        description = "MethodMesh does not store the credential. It records only that Android accepted the device credential before signing.",
                        confirmationRequired = true,
                        onSuccess = { authMethod ->
        ...
        activeDependency?.let { dependencyId ->
            CapabilityDependencyScreen(
                ...
                settings = if (dependencyId == "barcode.scan") {
                    supplied + mapOf("barcode_formats" to "QR_CODE")
                } else {
                    supplied
                },
        ...
        CapabilityScreenScaffold(
            ...
            if (focusedLaunch) {
                ...
                if (!external && eventPayloadHash == MANUAL_DEBUG_EVENT_PAYLOAD_HASH) {
                    Text(
                        "Manual test placeholder: SHA-256 of “methodmesh-manual-debug-event-v1”. Replace it when testing a specific payload.",
                        style = MaterialTheme.typography.bodySmall
                    )
                }
                Spacer(Modifier.height(8.dp))
...
private const val MANUAL_DEBUG_EVENT_PAYLOAD = "methodmesh-manual-debug-event-v1"
            ...
            IntentExampleDropdown(
                ...
                examples = listOf(
                    IntentExample(
                        ...
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='${As100CreateAttestationAnchorMethod.ID}')"
                    ),
                    IntentExample(
                        label = "With study context",
                        description = "Include study and operator information",
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='${As100CreateAttestationAnchorMethod.ID}',input_study_id='study_01',operator_id='op_001')"
                    )
                )
            )
        }
            ...
            if (publicKey.isNotBlank()) {
                Text(
                    ...
                    modifier = Modifier.combinedClickable(
                        onClick = { },
                        onLongClick = {
                            clipboard.setPrimaryClip(ClipData.newPlainText("MethodMesh public attestation key", publicKey))
                            copied = true
                        }
                    ),
