...
The capability is invoked through the standard MethodMesh execution action:

```text
com.example.methodmesh.EXECUTE_METHOD(method_id='android_app_inspector',return_mode='flat')
```
...
The accompanying `example_odk_android_app_inspector.xlsx` demonstrates invoking the capability through `com.example.methodmesh.EXECUTE_METHOD` and returning the flat result into an ODK group.

This is a diagnostic/integration-builder prototype. A later version can save a tested public intent as a reusable MethodMesh integration definition.
