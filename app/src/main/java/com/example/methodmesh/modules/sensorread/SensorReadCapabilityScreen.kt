...
object SensorReadCapabilityScreen : CapabilityScreenSpec {
    ...
    override val description = "Read a single value, trace, or average from a MethodMesh BLE sensor node."
    ...
    override fun Render(
                ...
                override fun onServicesDiscovered(g: BluetoothGatt, statusCode: Int) {
                    handler.post {
                        ...
                        if (statusCode != BluetoothGatt.GATT_SUCCESS || service == null || manifestCharacteristic == null || readingCharacteristic == null) {
                            produce(baseValues("failed") + mapOf(SensorReadFields.ERROR to "Connected device does not expose the MethodMesh sensor contract."), false)
                        } else {
        ...
        fun startScan(autoConnect: Boolean) {
            ...
            handler.postDelayed({
                ...
                when {
                    ...
                    else -> status = if (nearby.isEmpty()) "No MethodMesh sensors found nearby." else "Scan complete. Choose a sensor."
                }
            }, 8_000L)
        }
        ...
        CapabilityScreenScaffold(
            ...
            IntentExampleDropdown(
                ...
                examples = listOf(
                    IntentExample("Single sensor read", "Read a registered sensor once.", "com.example.methodmesh.EXECUTE_METHOD(method_id='sensor.read',input_device_id='clinic_room_01_sensor',input_sensor_read_mode='single',return_mode='flat')"),
                    IntentExample("Average over time", "Average AHT20 fields over 60 seconds.", "com.example.methodmesh.EXECUTE_METHOD(method_id='sensor.read',input_device_id='clinic_room_01_sensor',input_sensor_profile='aht20',input_sensor_read_mode='average',input_duration_seconds='60',input_sample_interval_seconds='5',return_mode='flat')"),
                    IntentExample("Strict device match", "Fail if the specified sensor is not nearby.", "com.example.methodmesh.EXECUTE_METHOD(method_id='sensor.read',input_device_id='clinic_room_01_sensor',input_device_match_policy='strict',return_mode='flat')")
                )
            )
        }
    }
...
private fun sensorProfileOptions(): List<Pair<String, String>> = listOf(
    ...
    "generic" to "Generic MethodMesh sensor payload"
)
