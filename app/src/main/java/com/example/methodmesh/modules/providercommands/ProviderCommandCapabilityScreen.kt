...
object ProviderCommandCapabilityScreen : CapabilityScreenSpec {
    ...
    override fun Render(
        ...
        var status by rememberSaveable { mutableStateOf("Choose a saved command, or teach MethodMesh a new one.") }
        ...
        CapabilityScreenScaffold(
            ...
            Text("Teach MethodMesh an app command once, save it under a stable name, then run it by name from forms, schedules, or the debug screen. MethodMesh also includes a small built-in command catalogue for common apps such as OsmAnd.", style = MaterialTheme.typography.bodyMedium)
            ...
            if (activeView == "run") {
                Row {
                    ...
                    OutlinedButton(onClick = {
                        ...
                        clipboard.setPrimaryClip(ClipData.newPlainText("MethodMesh provider command", payload))
                ...
                Row {
                    OutlinedButton(onClick = {
                        ...
                        clipboard.setPrimaryClip(ClipData.newPlainText("MethodMesh provider command registry", payload))
            ...
            IntentExampleDropdown(
                ...
                examples = listOf(
                    IntentExample(
                        ...
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='provider.command.run',input_provider_command_id='OsmAnd::navigate_to',input_latitude='-1.2864',input_longitude='36.8172',input_label='Clinic entrance',return_mode='flat')"
                    )
                )
            )
        }
...
private fun ProviderCommand.modeDescription(): String = when (interfaceType) {
    "activity_result_intent" -> "Returns data to MethodMesh when the external app finishes."
    "launch_only_intent" -> "Launch-only: opens the external app, but completion is not confirmed by MethodMesh."
