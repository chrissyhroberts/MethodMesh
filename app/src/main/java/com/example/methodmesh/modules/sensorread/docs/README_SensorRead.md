...
`sensor.read` connects to a registered MethodMesh BLE sensor node and returns
...
```text
com.example.methodmesh.EXECUTE_METHOD(method_id='sensor.read',input_device_id='clinic_room_01_sensor',input_sensor_id='radar_1',input_sensor_read_mode='trace',input_duration_seconds='120',input_sample_interval_seconds='1',return_mode='flat')
```
...
found, the default `fallback` policy opens a chooser of nearby MethodMesh sensor
