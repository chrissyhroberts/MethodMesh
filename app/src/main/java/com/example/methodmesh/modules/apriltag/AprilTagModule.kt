...
object AprilTagModule : MethodMeshModule {
    ...
    override fun rilBindings() = listOf(
        ...
        RilBinding("calibrate tag range", As100AprilTagCalibrateFocalMethod.id, "Calibrate real-world pose scale from a known-size tag at a known distance"),
    ...
    private val detectorSettings = listOf(
        ...
        MethodSetting.IntSetting(AprilTagInputs.THREADS, "Detector threads", "AprilTag detector worker threads.", "Detector", 2, 1, 8, runtimeInputAllowed = false),
        MethodSetting.FloatSetting(AprilTagInputs.QUAD_DECIMATE, "Quad decimation", "1 is most detailed and is the robust default for oblique/smaller tags; higher values are faster but reduce detection margin.", "Detector", 1f, 1f, 4f, 0.25f, decimals = 2, runtimeInputAllowed = false),
        MethodSetting.BooleanSetting(AprilTagInputs.REFINE_EDGES, "Refine edges", "Refine detected quadrilateral edges before decoding.", "Detector", true, runtimeInputAllowed = false)
    )
    ...
    private val metricScaleSettings = listOf(
        MethodSetting.FloatSetting(
            AprilTagInputs.DISTANCE_SCALE,
            "Distance adjustment",
            "Dimensionless metric correction. Corrected XYZ/range = raw pose × this value. Calibrate it with a known camera-to-tag distance.",
            "Metric calibration",
            AprilTagContractMetadata.DEFAULT_DISTANCE_SCALE,
            0.1f,
            10f,
            0.001f,
            "×",
    ...
    private val intrinsicsSettings = listOf(
        MethodSetting.ChoiceSetting(AprilTagInputs.INTRINSICS_MODE, "Camera model", "Use Android factory metadata when available, or supply a calibrated manual profile.", "Camera model", "auto", listOf("auto", "manual"), runtimeInputAllowed = false),
        MethodSetting.FloatSetting(AprilTagInputs.FX_PX, "fx", "Manual horizontal focal length in pixels.", "Manual camera model", 0f, 0f, 100000f, 1f, "px", 3, runtimeInputAllowed = false),
        MethodSetting.FloatSetting(AprilTagInputs.FY_PX, "fy", "Manual vertical focal length in pixels.", "Manual camera model", 0f, 0f, 100000f, 1f, "px", 3, runtimeInputAllowed = false),
        MethodSetting.FloatSetting(AprilTagInputs.CX_PX, "cx", "Manual principal point x in pixels.", "Manual camera model", 0f, 0f, 100000f, 1f, "px", 3, runtimeInputAllowed = false),
        MethodSetting.FloatSetting(AprilTagInputs.CY_PX, "cy", "Manual principal point y in pixels.", "Manual camera model", 0f, 0f, 100000f, 1f, "px", 3, runtimeInputAllowed = false),
        MethodSetting.IntSetting(AprilTagInputs.INTRINSICS_WIDTH_PX, "Calibration image width", "Pixel width to which the manual camera model applies.", "Manual camera model", 0, 0, 20000, 1, "px", runtimeInputAllowed = false),
        MethodSetting.IntSetting(AprilTagInputs.INTRINSICS_HEIGHT_PX, "Calibration image height", "Pixel height to which the manual camera model applies.", "Manual camera model", 0, 0, 20000, 1, "px", runtimeInputAllowed = false)
    )

    override fun capabilitySettings() = mapOf(
        As100AprilTagDetectMethod.id to detectorSettings,
        As100AprilTagCalibrateFocalMethod.id to detectorSettings + tagGeometrySettings + metricScaleSettings + intrinsicsSettings + listOf(
            MethodSetting.FloatSetting(AprilTagInputs.KNOWN_DISTANCE_MM, "Known camera-to-tag distance", "Measure from the camera optical centre to the centre of a front-facing tag as closely as practical.", "Calibration", 1000f, 50f, 20000f, 1f, "mm", 1),
            MethodSetting.IntSetting(AprilTagInputs.CALIBRATION_SAMPLES, "Samples", "Number of stable pose observations to average before calculating the distance adjustment.", "Calibration", 15, 3, 100, runtimeInputAllowed = false)
        ),
        As100AprilTagRangePoseMethod.id to detectorSettings + tagGeometrySettings + metricScaleSettings + intrinsicsSettings,
        As100AprilTagRelativePoseMethod.id to detectorSettings.filterNot { it.id == AprilTagInputs.TARGET_TAG_ID } + tagGeometrySettings + metricScaleSettings + intrinsicsSettings + listOf(
        ...
        As100AprilTagPlanarMeasureMethod.id to detectorSettings + tagGeometrySettings + listOf(
            MethodSetting.ChoiceSetting(AprilTagInputs.MEASUREMENT_MODE, "Measurement", "Choose what tapping points on the tagged plane means.", "Experiment", "point", listOf("point", "distance", "area", "trajectory"), runtimeInputAllowed = false),
        ...
        As100AprilTagTrackPoseMethod.id to detectorSettings.filterNot { it.id == AprilTagInputs.TARGET_TAG_ID } + tagGeometrySettings + metricScaleSettings + intrinsicsSettings + listOf(
            ...
            MethodSetting.IntSetting(AprilTagInputs.SAMPLE_INTERVAL_MS, "Sample interval", "Minimum time between recorded pose samples.", "Tracking", 200, 50, 10000, 50, "ms", runtimeInputAllowed = false),
            MethodSetting.IntSetting(AprilTagInputs.MAX_SAMPLES, "Maximum samples", "Stops recording before an unbounded in-memory trajectory develops.", "Tracking", 3000, 2, 100000, runtimeInputAllowed = false)
        )
    )
