...
## Arcade v0.02 restoration

Arcade is again an independent capability alongside GameDeck.

Current executable slice:

- Snake Sprint — solo, score/length records, secure/fixed-seed Chance-backed food placement.
- Pong Table — CPU or two humans, first to seven, fixed-step real-time physics.

Shared principles now proven on both capability families:

- generic immersive hosting;
- local-first execution;
- capability-owned UX;
- visible non-hidden primary controls;
- modal pause;
- explicit end-state actions;
- state transitions authoritative over animation.

Next cross-capability architecture step remains extraction of a public game-system identity / record / replay boundary. Do not solve that by importing GameDeck internals into Arcade or vice versa.
