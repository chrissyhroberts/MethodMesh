            ...
            IntentExampleDropdown(
                ...
                examples = listOf(
                    IntentExample(
                        ...
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='calibrated_scale',input_prompt='Rate your pain',input_hint='0 means no pain; 100 means the worst pain you can imagine')"
                    ),
                    IntentExample(
                        label = "Custom range (0-10)",
                        description = "Measure on a custom range",
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='calibrated_scale',input_minimum='0',input_maximum='10')"
                    ),
                    IntentExample(
                        label = "Dual range measurement",
                        description = "Capture both lower and upper values",
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='calibrated_scale',input_use_range='true',input_lower_label='Minimum pain',input_upper_label='Maximum pain')"
                    ),
                    IntentExample(
                        label = "Calibrated 5 cm horizontal scale",
                        description = "Display a physically calibrated 50 mm line",
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='calibrated_scale',input_vas_length_mm='50',input_vertical_mode='false')"
                    ),
                    IntentExample(
                        label = "Calibrated 5 cm vertical scale",
                        description = "Display the same physical length vertically",
                        intentUri = "com.example.methodmesh.EXECUTE_METHOD(method_id='calibrated_scale',input_vas_length_mm='50',input_vertical_mode='true')"
                    )
                )
            )
        }
