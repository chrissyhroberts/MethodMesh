package com.example.methodmesh.platform.timestamp
...
object TrustedTimestampEngine {
    private const val TIMESTAMPING_EKU_OID = "1.3.6.1.5.5.7.3.8"
    ...
    fun parseAndValidate(
        ...
        val publishedSignerCertificate = publishedSigner?.pem?.let(::parseCertificate)
        val publishedRootCertificate = publishedRoot?.pem?.let(::parseCertificate)
        val packagedSignerPem = publishedSigner?.pem ?: signerCertificate?.toPem()
        val packagedSignerCertificate = packagedSignerPem?.let(::parseCertificate) ?: signerCertificate
        ...
        val embeddedSignerMatchesPublished =
            signerCertificate != null && publishedSignerCertificate != null &&
                signerCertificate.encoded.contentEquals(publishedSignerCertificate.encoded)
        val chainValid = runCatching {
            requireNotNull(publishedSignerCertificate).verify(requireNotNull(publishedRootCertificate).publicKey)
            publishedRootCertificate.verify(publishedRootCertificate.publicKey)
            true
        }.getOrDefault(false)
        val certificateTimeValid = runCatching {
            val generationDate = info.genTime
            requireNotNull(publishedSignerCertificate).checkValidity(generationDate)
            requireNotNull(publishedRootCertificate).checkValidity(generationDate)
            true
        }.getOrDefault(false)
        val timestampEkuValid = publishedSignerCertificate?.let { certificate ->
            certificate.extendedKeyUsage == listOf(TIMESTAMPING_EKU_OID) &&
                certificate.criticalExtensionOIDs?.contains(EXTENDED_KEY_USAGE_OID) == true
        } == true
        val signingKeyUsageValid = publishedSignerCertificate?.keyUsage?.let { usage ->
            (usage.getOrNull(0) == true) || (usage.getOrNull(1) == true)
        } ?: true

        val trustStatus = when {
            !signatureValid -> "signature_invalid"
            authority == null -> "cryptographically_valid_unconfigured_trust"
            signerPinOk != true || rootPinOk != true -> "trusted_registry_mismatch"
            !embeddedSignerMatchesPublished -> "signer_certificate_mismatch"
            !chainValid -> "certificate_chain_invalid"
            !certificateTimeValid -> "certificate_time_invalid"
            !timestampEkuValid || !signingKeyUsageValid -> "timestamp_certificate_usage_invalid"
            else -> "trusted_registry_match"
        }
        ...
        return TrustedTimestampEvidence(
            ...
            rootCertificate = publishedRootCertificate?.toTimestampCertificateInfo(),
