...
object SchedulerExportCapabilityScreen : CapabilityScreenSpec {
    ...
    override fun Render(context: CapabilityScreenContext, onBack: () -> Unit, onConfirmed: (ExecutionResult) -> Unit, onCancel: () -> Unit) {
        ...
        CapabilityScreenScaffold(title, capabilityId, context, context.stepNumber > 1, result, result?.let { OutputFormatter.fields(it, false) }.orEmpty(), onBack, {}, { result?.let(onConfirmed) }, onCancel) {
            ...
            Button(onClick = {
                androidContext.getSystemService(ClipboardManager::class.java).setPrimaryClip(ClipData.newPlainText("ResearchOS schedule bundle", payload))
            }, modifier = Modifier.fillMaxWidth()) { Text("Copy bundle to clipboard") }
            Button(onClick = { fileLauncher.launch("researchos-schedules.json") }, modifier = Modifier.fillMaxWidth()) { Text("Save bundle as file") }
            Button(onClick = {
            ...
            Button(onClick = { qrFileLauncher.launch("researchos-schedule-qr.png") }, enabled = qrBitmap != null, modifier = Modifier.fillMaxWidth()) { Text("Save QR code image") }
            Button(onClick = {
                qrBitmap?.let { bitmap ->
                    val file = File(androidContext.cacheDir, "researchos-schedule-qr.png")
...
private object SchedulerTransferResults {
    ...
    private fun result(context: CapabilityScreenContext, method: String, values: Map<String, Any?>, failed: Boolean = false): ExecutionResult {
        ...
        val provenance = ProvenanceContext(provider = "researchos.scheduler", methodId = method, methodVersion = "1.0.0")
        val observation = Observation(phenomenon = "researchos.schedule_transfer", subject = null, values = values.mapValues { it.value?.toString().orEmpty() }, temporalContext = request.temporalContext, provenance = provenance)
