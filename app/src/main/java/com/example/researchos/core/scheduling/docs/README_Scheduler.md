...
The scheduler creates local recurring alarms for an ODK/Kobo form, an online web form, or a future ResearchOS process target. It supports daily, weekly, monthly, and nth-weekday rules, with notification reminders and configurable retry intervals.

## Android intent

```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.create',schedule_name='Daily check',schedule_target='ODK_FORM',schedule_target_value='my_form_id',input_project_id='my_project_id',schedule_frequency='DAILY',schedule_time='09:00',schedule_retry_count='2',schedule_retry_interval_minutes='60',return_mode='flat')
```
...
```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.export',return_mode='flat')
```

Export one schedule:

```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.export',schedule_id='SCHEDULE_ID',return_mode='flat')
```
...
```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.import',schedule_bundle='URL_SAFE_OR_JSON_BUNDLE',return_mode='flat')
```

Import via QR or NFC uses the same generic dependency boundary as other chained capabilities:

```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.import',schedule_transport='QR',return_mode='flat')
```

or:

```text
com.example.researchos.EXECUTE_METHOD(method_id='scheduler.import',schedule_transport='NFC',return_mode='flat')
```
