...
private fun selectorSvgDocument(svg: String, mode: String): String {
    ...
    val root = renderSource.replaceFirst(Regex("<svg\\b", RegexOption.IGNORE_CASE), "<svg id=\"researchos-root\"")
    val script = """
        ...
        function point(p){const svg=document.getElementById('researchos-root');if(!svg||!svg.viewBox.baseVal)return{x:.5,y:.5};const r=p.getBoundingClientRect(),s=svg.getBoundingClientRect();return{x:Math.max(0,Math.min(1,(r.left+r.width/2-s.left)/s.width)),y:Math.max(0,Math.min(1,(r.top+r.height/2-s.top)/s.height))}}
        function emit(message){ResearchOS.update(JSON.stringify(selected.map((x,i)=>({polygon_id:x.id,sequence_index:i+1}))),JSON.stringify(events),message)}
        function redraw(){document.querySelectorAll('.picked').forEach(x=>x.classList.remove('picked'));selected.forEach(x=>document.getElementById(x.id)?.classList.add('picked'));document.getElementById('researchos-labels')?.remove();if(mode!=='sequence')return;const svg=document.getElementById('researchos-root');if(!svg)return;const g=document.createElementNS('http://www.w3.org/2000/svg','g');g.id='researchos-labels';const vb=svg.viewBox.baseVal;selected.forEach((x,i)=>{const t=document.createElementNS('http://www.w3.org/2000/svg','text');t.setAttribute('x',String(x.x*vb.width));t.setAttribute('y',String(x.y*vb.height));t.setAttribute('class','selection-label');t.textContent=String(i+1);g.appendChild(t)});svg.appendChild(g)}
    ...
    """.trimIndent()
    val injection = "<style><![CDATA[html,body{margin:0;background:#fff;overflow:auto}#researchos-root{width:100%;height:100%;touch-action:auto}path,polygon,rect,circle,ellipse{cursor:pointer;transition:fill .12s,stroke .12s}.picked{fill:#ff8a65!important;stroke:#8d2f1f!important;stroke-width:4}.selection-label{font:700 24px sans-serif;fill:#0d47a1;stroke:#fff;stroke-width:5px;paint-order:stroke;pointer-events:none}]]></style><script type=\"text/javascript\"><![CDATA[$script]]></script>"
