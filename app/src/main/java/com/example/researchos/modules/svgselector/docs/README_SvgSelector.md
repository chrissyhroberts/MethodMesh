...
`svg.select` opens an SVG stored in ResearchOS app storage and lets the caller choose one polygon, several polygons, or an ordered polygon sequence.
...
```text
com.example.researchos.EXECUTE_METHOD(method_id='svg.select',input_svg_name='bodymap_black.svg',input_selection_mode='sequence',return_mode='flat')
```
