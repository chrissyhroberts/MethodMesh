...
This capability installs the bundled ResearchOS ESP32-C3 sensor-node firmware stack from inside the Android app. It is intended to remove the need for `esptool`, `mpremote`, or a laptop during field provisioning.

## Current scope

Supported now:

- ResearchOS carries the official MicroPython `ESP32_GENERIC_C3-20260406-v1.28.0.bin` as an Android asset.
- ResearchOS carries the bundled sensor-node `main.py` and sensor driver files
  as Android assets.
- The phone lists connected USB devices.
- The user grants USB permission.
- ResearchOS only enables raw flashing for recognised Espressif or USB-serial targets.
- ResearchOS requires explicit confirmation before overwriting firmware.
- ResearchOS speaks a small ESP32 ROM bootloader protocol to write MicroPython
  at flash address `0`.
- After the board has restarted into MicroPython, ResearchOS uploads the bundled
  ResearchOS Python files over the friendly REPL.
...
7. Tap **Install ResearchOS main.py**. This uploads `main.py` and the bundled
   sensor driver files.
...
```text
com.example.researchos.EXECUTE_METHOD(method_id='sensor_firmware_installer',return_mode='flat')
```
...
| `firmware_version` | ResearchOS firmware version. |
