...
object ImageRedactionCapabilityScreen : CapabilityScreenSpec {
    ...
    override fun Render(
        ...
        fun startCamera() {
            val file = File(appContext.cacheDir, "researchos-redaction-source-${System.currentTimeMillis()}.jpg")
        ...
        fun finish() {
            ...
            val file = File(appContext.cacheDir, "researchos-redacted-${System.currentTimeMillis()}.jpg")
        ...
        CapabilityScreenScaffold(
            ...
            IntentExampleDropdown(
                capabilityId = capabilityId,
                examples = listOf(
                    IntentExample("Redact from camera", "Capture an image and mask selected cells.", "com.example.researchos.EXECUTE_METHOD(method_id='image.redact',input_source='camera',input_grid_rows='10',input_grid_columns='10',return_mode='flat')"),
                    IntentExample("Redact picked image", "Pick an image and mask selected cells.", "com.example.researchos.EXECUTE_METHOD(method_id='image.redact',input_source='file_picker',input_grid_rows='10',input_grid_columns='10',return_mode='flat')")
                )
            )
        }
    }
    ...
    Box(Modifier.fillMaxWidth().height(440.dp)) {
        ...
        Canvas(Modifier.fillMaxSize()) {
            ...
            for (r in 0 until rows) {
                for (c in 0 until columns) {
                    val id = "r${r + 1}c${c + 1}"
                    val topLeft = Offset(c * cellWidth, r * cellHeight)
        ...
        Box(Modifier.fillMaxSize().onSizeChanged { boxSize = it }.pointerInput(rows, columns, selected, boxSize) {
            awaitEachGesture {
                ...
                var updated = toggleCell(selected, cellAt(down.position, boxSize, rows, columns))
                var dragged = false
                drag(down.id) { change ->
                    dragged = true
                    change.consume()
                    cellAt(change.position, boxSize, rows, columns)?.let { updated = updated + it }
                }
                onChanged(updated)
            }
        })
...
private fun cellAt(position: Offset, size: IntSize, rows: Int, columns: Int): String? {
    if (size.width <= 0 || size.height <= 0) return null
    if (position.x !in 0f..size.width.toFloat() || position.y !in 0f..size.height.toFloat()) return null
    val column = ((position.x / size.width) * columns).toInt().coerceIn(0, columns - 1)
