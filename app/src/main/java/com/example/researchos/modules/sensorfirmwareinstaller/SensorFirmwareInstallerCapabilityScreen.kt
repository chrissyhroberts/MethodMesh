...
private const val ACTION_USB_PERMISSION = "com.example.researchos.USB_SENSOR_FIRMWARE_PERMISSION"
...
object SensorFirmwareInstallerCapabilityScreen : CapabilityScreenSpec {
    ...
    override val description = "Install MicroPython and bundled ResearchOS sensor firmware to an ESP32-C3 over USB."
    ...
    override fun Render(context: CapabilityScreenContext, onBack: () -> Unit, onConfirmed: (ExecutionResult) -> Unit, onCancel: () -> Unit) {
        ...
        fun installMainPy(device: UsbDevice) {
            ...
            installing = true
            result = null
            status = "Installing bundled ResearchOS main.py…"
            ...
            scope.launch {
                ...
                installing = false
                status = if (outcome.success) "ResearchOS main.py installed. Reset the board, then run BLE sensor provisioning." else "ResearchOS main.py install failed: ${outcome.message}"
                ...
                record(
                    SensorFirmwareInstallOutcome(
                        ...
                        firmwareVersion = "researchos-sensor-0.1.1",
        ...
        fun flashMicroPython(device: UsbDevice, cleanErase: Boolean = false) {
            ...
            scope.launch {
                ...
                status = if (outcome.success) {
                    if (cleanErase) "Clean MicroPython install complete. Reset or unplug/replug the board, refresh USB devices, then install ResearchOS main.py."
                    else "MicroPython installed. Reset or unplug/replug the board, refresh USB devices, then install ResearchOS main.py."
        ...
        fun installCompleteFirmware(device: UsbDevice) {
            ...
            log = "Opening ESP32 ROM bootloader on ${device.deviceName}\nThis step writes MicroPython only. After it succeeds, reset or unplug/replug the board, refresh USB devices, then press Upload ResearchOS main.py."
            scope.launch {
                val outcome = withContext(Dispatchers.IO) {
                ...
                installing = false
                status = if (outcome.success) {
                    "Step 1 complete. Reset or unplug/replug the board normally, refresh USB devices, then press Upload ResearchOS main.py."
                } else {
        ...
        CapabilityScreenScaffold(
            ...
            Text("Install the ResearchOS sensor stack to an ESP32-C3 from the phone.", style = MaterialTheme.typography.bodyMedium)
            Text("Step 1 flashes MicroPython in ESP bootloader mode. Step 2 uploads the bundled ResearchOS main.py after the board has rebooted normally.", style = MaterialTheme.typography.bodySmall)
            ...
            Button(onClick = { selected?.let { installMainPy(it) } ?: run { status = "No USB device selected." } }, modifier = Modifier.fillMaxWidth(), enabled = !installing) {
                Text(if (installing) "Working…" else "Step 2: upload ResearchOS main.py")
            }
            Spacer(Modifier.height(8.dp))
            ...
            IntentExampleDropdown(
                ...
                examples = listOf(IntentExample("Install bundled firmware", "Open the USB firmware installer.", "com.example.researchos.EXECUTE_METHOD(method_id='sensor_firmware_installer',return_mode='flat')"))
            )
        }
    }
}
...
private fun installMainPyFirmware(
    ...
    serial.use {
        ...
        runCatching { rawExec(it, "print('ResearchOS MicroPython link ready')", "raw probe") }
...
private fun friendlyReplProbe(serial: PythonSerialLink): String {
    ...
    serial.write("print('ResearchOS friendly REPL ready')\r\n".toByteArray(Charsets.UTF_8))
    val response = serial.readAvailable(4500)
    if (!response.contains("ResearchOS friendly REPL ready")) {
        throw IllegalStateException(
...
private fun flashMicroPythonImage(usbManager: UsbManager, device: UsbDevice, image: ByteArray, cleanErase: Boolean = false): FirmwareInstallResult {
    ...
    return FirmwareInstallResult(
        true,
        device.usbLabel(),
        if (cleanErase) "MicroPython v1.28.0 was clean-installed after erasing the normal 4 MB flash area. Reset the board, then install ResearchOS main.py."
        else "MicroPython v1.28.0 was written at flash address 0. Reset the board, then install ResearchOS main.py."
    )
}
...
private class Esp32RomBootloader(private val serial: UsbSerialLink) {
    fun sync() {
        ...
        if (!ok) throw IllegalStateException("ESP32 ROM bootloader did not respond. For a blank/wiped board, hold BOOT, tap RESET, release BOOT, then press Install complete ResearchOS sensor firmware again. If it still times out, unplug/replug while holding BOOT, grant USB permission again, then retry.")
    }
