...
object OutputExportRepository {
    ...
    fun exportFlatPackage(
        ...
        val json = JSONObject().apply {
            put("researchos_output_schema", "researchos.output.package")
            put("researchos_export_version", "2")
            put("export_kind", "single_run")
            put("export_package_id", packageId)
            put("researchos_submission_id", packageId)
            put("export_package_name", folderName)
            put("exported_at", exportedAt)
            put("researchos_method_id", methodId)
            put("researchos_status", status)
            put("step_count", 1)
    ...
    fun exportProtocolStepPackage(
        ...
        existing.put("researchos_submission_id", existing.optString("researchos_submission_id").ifBlank { packageId })
        ...
        nextSteps.put(JSONObject().apply {
            ...
            put("researchos_submission_id", existing.optString("researchos_submission_id", packageId))
            put("researchos_method_id", methodId)
            put("researchos_status", status)
            put("exported_at", exportedAt)
            put("fields", jsonFields)
    ...
    fun exportPackage(context: Context, result: ExecutionResult): ExportPackage {
        ...
        val timestamp = stamp.format(Instant.now())
        val packageId = UUID.randomUUID().toString()
        val method = safeSegment(result.request.method.id.value).ifBlank { "researchos_method" }
    ...
    fun share(context: Context, exportPackage: ExportPackage) {
        ...
        val intent = if (uris.size == 1) {
            ...
            putExtra(Intent.EXTRA_SUBJECT, "ResearchOS export")
            putExtra(Intent.EXTRA_TEXT, "ResearchOS export: ${exportPackage.summary}")
        }
    ...
    fun notifySaved(context: Context, exportPackage: ExportPackage) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            manager.createNotificationChannel(
                ...
                ).apply { description = "Saved ResearchOS result packages" }
            )
        }
        ...
        val notification = NotificationCompat.Builder(context, channelId)
            ...
            .setContentTitle("ResearchOS output saved")
    ...
    private fun findDefaultDocument(context: Context, folderName: String, name: String): Uri? {
        ...
        val relativePath = Environment.DIRECTORY_DOCUMENTS + "/ResearchOS/outputs/$folderName/"
    ...
    private fun writeDefault(context: Context, folderName: String, name: String, mime: String, bytes: ByteArray): String? {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            val values = android.content.ContentValues().apply {
                ...
                put(MediaStore.Files.FileColumns.RELATIVE_PATH, Environment.DIRECTORY_DOCUMENTS + "/ResearchOS/outputs/$folderName")
    ...
    private fun defaultOutputsDocumentUri(): Uri = Uri.parse(
        "content://com.android.externalstorage.documents/document/primary%3ADocuments%2FResearchOS%2Foutputs"
    )

    private fun defaultOutputFolderDocumentUri(folderName: String): String {
        val documentId = Uri.encode("primary:Documents/ResearchOS/outputs/$folderName")
    ...
    fun defaultOutputsPathLabel(folderName: String? = null): String =
        listOf("Documents", "ResearchOS", "outputs", folderName.orEmpty()).filter { it.isNotBlank() }.joinToString("/")
