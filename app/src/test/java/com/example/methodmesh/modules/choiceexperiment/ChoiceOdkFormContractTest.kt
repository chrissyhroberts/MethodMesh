...
class ChoiceOdkFormContractTest {
    ...
    fun `choice forms carry static configuration in namespaced intent inputs`() {
        val docs = listOf(
            File("src/main/java/com/example/methodmesh/modules/choiceexperiment/docs"),
            File("app/src/main/java/com/example/methodmesh/modules/choiceexperiment/docs")
        ...
        workbooks.forEach { (workbook, methodId) ->
            ...
            assertTrue(workbook.name, "com.example.methodmesh.EXECUTE_METHOD" in xml)
            assertTrue(
                "${workbook.name} must route to its fixed method",
                "com.example.methodmesh.EXECUTE_METHOD(method_id='$methodId'," in xml
            )
