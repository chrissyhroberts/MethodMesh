...
class ModuleBoundaryTest {
    ...
    fun `core imports no module layer`() {
        val coreRoot = File(sourceRoot, "com/example/researchos/core")
        val violations = coreRoot.walkTopDown().filter { it.extension == "kt" }.flatMap { file ->
            file.useLines { lines ->
                lines.filter { it.trim().startsWith("import com.example.researchos.modules.") }
    ...
    fun `capability modules import no sibling capability implementation`() {
        val moduleRoot = File(sourceRoot, "com/example/researchos/modules")
            ...
            .flatMap { module ->
                module.walkTopDown().filter { it.extension == "kt" }.flatMap { file ->
                    concreteModuleImports(file)
                        .filterNot { it.startsWith("com.example.researchos.modules.${module.name}.") }
                        .map { "${file.path}: $it" }
                }.toList()
            }
    ...
    fun `parallel research model and central manifest are absent`() {
        assertFalse(File(sourceRoot, "com/example/researchos/core/research/ResearchRecords.kt").exists())
        assertFalse(File(sourceRoot, "com/example/researchos/modules/ResearchOSModuleManifest.kt").exists())
    }

    private fun concreteModuleImports(file: File): Sequence<String> = file.useLines { lines ->
        lines.mapNotNull { line ->
            Regex("^import (com\\.example\\.researchos\\.modules\\.[^.]+\\..+)$")
