...
DESKTOP_VERIFIER_ID = "methodmesh_audit_v15_guided_auditor"
METHODMESH_AUDIT_VERSION = "15.0"
...
<dl class="kv" id="proof-kv"></dl><h3 style="margin-top:16px">Independent verification</h3>
<pre>python3 verify_bundle.py</pre></div></section>
</main>
...
def scope_analytics_preview_to_project(
    ...
    """
    Preserve installation-wide aggregate system metrics, but retain only the
    selected project's row from the analytics per-project array.
    """
    ...
    if "system" in analytics:
        result["system"] = analytics.get("system")
    ...
    if isinstance(projects, list):
        result["projects"] = [
            ...
            and _coerce_project_id(item.get("id")) == int(project_id)
        ]
