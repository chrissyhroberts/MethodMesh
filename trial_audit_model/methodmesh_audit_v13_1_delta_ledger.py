...
DESKTOP_VERIFIER_ID = "methodmesh_audit_v13_1_delta_ledger"
...
def persist_form_metadata(
    ...
    *,
    ...
    forms:
        current logical-form state, keyed by xmlFormId. If an active form
        exists, it always wins over any deleted historical form with the same
        xmlFormId.

    form_instances:
        every Central form object we can identify by numeric form ID,
        including deleted historical instances.
    """
    active_ids = {
        item.get("xmlFormId")
        for item in forms
    ...
    def upsert_instance(item: dict) -> None:
    ...
    # Active forms first and authoritative for current logical-form state.
    for item in forms:
        form_id = item.get("xmlFormId")
        if not form_id:
            continue
    ...
    # Deleted historical instances are retained, but cannot overwrite a
    # currently active form that reuses the same xmlFormId.
...
def mark_deleted_submission(
    ...
    *,
    ...
    if prior is None:
        ...
        status = "DELETED"
        note = "Previously observed submission is now soft-deleted in Central."
