...
DESKTOP_VERIFIER_ID = "odk_version_probe_v12_auditor_safe_export"
...
def build_expected_leaves(evidence):
    ...
    for submission in evidence.get("submissions") or []:
        leaves.append(
            evidence_leaf(
                "submission",
                submission["logical_submission_id"],
                submission,
            )
        )
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Verify MethodMesh/ODK provenance, RFC3161-timestamp reviewed "
            "desktop receipts, and optionally create a privacy-safe daily "
            "Central/system evidence snapshot with field names only, service "
            "probes, a Merkle root and an RFC3161 daily checkpoint."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Create a privacy-safe daily trial/system evidence snapshot, "
            "Merkle root and RFC3161 timestamp after verification."
        ),
    )
    parser.add_argument(
        "--probe-only",
        action="store_true",
        help=(
            "Run only the point-in-time DNS/TLS/HTTP/API/TCP service probe "
            "and append it to the local probe log. Intended for a separate "
            "high-frequency scheduler if continuous availability evidence "
            "is desired."
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Comma-separated explicit TCP ports to test. This is a targeted "
            "exposure check, not a general port scan."
        ),
    )
    parser.add_argument(
        ...
        default="80,443",
        help=(
            "Comma-separated ports expected to be externally reachable. "
            "Unexpected open ports are flagged in daily evidence."
        ),
    )
    parser.add_argument(
        "--auditor-export-dir",
        help=(
            "Directory for shareable auditor-safe ZIPs. Defaults to a "
            "sibling '<output>_auditor_exports' directory so it cannot be "
            "confused with the restricted provenance archive."
        ),
    )
    parser.add_argument(
        "--writeback",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "v7 directory so v12 reuses previously archived immutable XML, "
    ...
    restricted_warning = (
        output_root / "RESTRICTED_ARCHIVE_DO_NOT_SHARE.txt"
    )
    restricted_warning.write_text(
        "METHODMESH RESTRICTED VERIFICATION ARCHIVE\n"
        "=========================================\n"
        "This directory may contain exact historical submission XML, "
        "attachments/images and detailed verifier working files.\n"
        "DO NOT provide this directory as the routine auditor package.\n"
        "Use the separately generated methodmesh_auditor_bundle_*.zip.\n",
        encoding="utf-8",
    )

    auditor_export_root = (
        Path(args.auditor_export_dir).expanduser()
        if args.auditor_export_dir
        else output_root.parent / f"{output_root.name}_auditor_exports"
    )
    ...
    expected_public_ports = parse_port_list(args.expected_public_ports)

    current_service_probe = service_probe(
        central=central,
    ...
    append_service_probe(
        output_root=output_root,
        probe=current_service_probe,
    )

    if args.probe_only:
        print(pretty_json(current_service_probe))
        print(
            f"Probe appended to: "
            f"{(output_root / '_service_probes.jsonl').resolve()}"
        )
        return
    ...
    if args.daily_checkpoint:
        daily_evidence = create_daily_evidence_snapshot(
            central=central,
            project_id=args.project,
            form_id=args.form,
            output_root=output_root,
            results=results,
            current_probe=current_service_probe,
            tsa_ca_file=args.tsa_ca_file,
            tsa_cert_file=args.tsa_cert_file,
            allow_tsa_download=not args.no_tsa_download,
        ...
        print()
        print("=" * 78)
        print("DAILY EVIDENCE CHECKPOINT")
        print("=" * 78)
        print(
            "Merkle root:      "
            f"{daily_evidence.get('evidence_merkle_root')}"
        )
        print(
            "Checkpoint SHA:   "
            f"{daily_evidence.get('daily_checkpoint_sha256')}"
        )
        print(
            "RFC3161:          "
            f"{daily_evidence.get('timestamp_status')}"
        )
        if daily_evidence.get("timestamp_time_iso"):
            print(
                "TSA time:         "
                f"{daily_evidence.get('timestamp_time_iso')}"
            )
        print(
            "Restricted daily evidence: "
            f"{daily_evidence.get('directory')}"
        )

        auditor_export = create_auditor_safe_export(
            evidence=daily_evidence.pop(
                "_daily_evidence_object_for_safe_export"
            ),
            service_probes=daily_evidence.pop(
                "_service_probes_for_safe_export"
            ),
            daily_dir=Path(daily_evidence["directory"]),
            output_root=output_root,
            export_root=auditor_export_root,
        )
        daily_evidence["auditor_export"] = auditor_export

        print(
            "SAFE AUDITOR ZIP: "
            f"{auditor_export.get('zip_path')}"
        )
        print("Safety scan:      PASS")
        print("Research values:  NOT EXPORTED")
        print("Attachments:      NOT EXPORTED")
    ...
    summary = {
        ...
        "privacy_policy": {
            "central_diffs_endpoint_queried": False,
            "daily_auditor_evidence_contains_research_values": False,
            "changed_fields_are_names_only": True,
        },
        "service_probe": current_service_probe,
