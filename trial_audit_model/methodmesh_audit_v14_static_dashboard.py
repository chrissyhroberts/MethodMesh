...
DESKTOP_VERIFIER_ID = "methodmesh_audit_v14_static_dashboard"
METHODMESH_AUDIT_VERSION = "14.0"
...
addKV($("coverage-kv"),"Metadata endpoints available",`${endpointEntries.filter(([,v])=>v&&v.available).length} / ${endpointEntries.length}`);addKV($("coverage-kv"),"Server audit endpoint",centralProvenance?"AVAILABLE":"UNAVAILABLE");addKV($("coverage-kv"),"Server audit events in window",serverAudits.length);addKV($("coverage-kv"),"Evidence leaf count",C.leaf_count);
...
$("system-cards").innerHTML=[card("Central audit",centralProvenance?"AVAILABLE":"LIMITED"),card("Native audit events",serverAudits.length),card("System endpoints",endpointEntries.filter(([,v])=>v&&v.available).length),card("Analytics preview",(endpoints.analytics_preview||{}).available?"CAPTURED":"UNAVAILABLE")].join("");
