...
class Central:
    def __init__(
        ...
        if token:
            ...
            console_status(
                "Using supplied Central bearer token. "
                "Authentication will be confirmed by the first API request."
            )
            return
        ...
        console_status(
            f"Authenticating to Central as {email}..."
        )
        ...
        if password is None:
            try:
                ...
                raise RuntimeError(
                    "Password entry was cancelled before authentication."
                )
        ...
        try:
            ...
            raise RuntimeError(
                "Central authentication timed out after 60 seconds. "
                "Check network/VPN connectivity and the Central URL."
            ) from exc
        except requests.exceptions.ConnectionError as exc:
            raise RuntimeError(
                "Could not connect to Central while authenticating. "
                "Check the Central URL, DNS/network connectivity, VPN and TLS."
            ) from exc
        except requests.RequestException as exc:
            raise RuntimeError(
                f"Central authentication request failed: {exc}"
            ) from exc
        ...
        if response.status_code in {401, 403}:
            raise RuntimeError(
                f"Central authentication failed (HTTP {response.status_code}) "
                f"for {email}. Check the email/password and that the account "
                "is permitted to sign in."
            )
        ...
        if not response.ok:
            ...
            raise RuntimeError(
                ...
                + (f"Server response: {body}" if body else "")
            )
        ...
        try:
            ...
            raise RuntimeError(
                "Central accepted the authentication request but did not "
                "return the expected session token."
            ) from exc
        ...
        console_status(
            f"Central authentication: PASS ({elapsed:.1f}s)"
        )
    ...
    def get_json(self, path: str, extended: bool = False):
        ...
        try:
            ...
            raise RuntimeError(
                f"Central GET timed out after 120s: {path}"
            ) from exc
        except requests.exceptions.ConnectionError as exc:
            raise RuntimeError(
                f"Central GET connection failed: {path}"
            ) from exc

        if response.status_code in {401, 403}:
            raise RuntimeError(
                f"Central API authorization failed "
                f"(HTTP {response.status_code}) while reading {path}. "
                "The session may have expired or the account may lack "
    ...
    def get_bytes(self, path: str):
        try:
            ...
            raise RuntimeError(
                f"Central download timed out after 120s: {path}"
            ) from exc

        if response.status_code in {401, 403}:
            raise RuntimeError(
                f"Central API authorization failed "
                f"(HTTP {response.status_code}) while downloading {path}."
            )
    ...
    def stream_sha256(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                f"Central attachment stream timed out after 120s: {path}"
            ) from exc
        ...
        if response.status_code in {401, 403}:
            ...
            raise RuntimeError(
                f"Central API authorization failed "
                f"(HTTP {response.status_code}) while streaming {path}."
            )
        ...
        try:
            for chunk in response.iter_content(chunk_size=chunk_size):
                ...
                if (
                    ...
                    and now - last_feedback >= 2.0
                ):
                    elapsed = max(0.001, now - started)
                    print(
                        f"\rStreaming attachment: "
                        f"{total / (1024 * 1024):,.1f} MiB "
                        f"({total / elapsed / (1024 * 1024):,.1f} MiB/s)",
                        end="",
                        flush=True,
                    )
                    last_feedback = now
    ...
    def put_xml(self, path: str, xml_bytes: bytes):
        ...
        if response.status_code in {401, 403}:
            raise RuntimeError(
                f"Central API authorization failed "
                f"(HTTP {response.status_code}) while writing {path}."
            )
        response.raise_for_status()
    ...
    def patch_json(self, path: str, payload: dict):
        ...
        if response.status_code in {401, 403}:
            raise RuntimeError(
                f"Central API authorization failed "
                f"(HTTP {response.status_code}) while updating {path}."
            )
        response.raise_for_status()
...
# System/config APIs can expose generic containers called "value", "old",
# "new", "payload", etc. These names are too ambiguous for an auditor-safe
# package: they could contain configuration values, research values, or
# arbitrary document content. Preserve evidence that something existed by
# hashing it, but never retain the raw content under these keys.
SYSTEM_METADATA_HASH_ONLY_KEYS = {
    "old", "new", "value", "values", "xml", "body", "payload", "content",
    "submission_xml", "source_record_fields",
    "showcase_payload_canonical",
    "attestation_methodmesh_full_json", "methodmesh_full_json",
    "attachment_bytes", "file_bytes", "binary", "base64_image",
}


def _metadata_hash_value(value) -> str:
    try:
...
def sanitize_system_metadata(value, *, key_hint: str | None = None):
    ...
    if isinstance(value, dict):
        ...
        for k, v in value.items():
            ...
            if key_lower in SYSTEM_METADATA_HASH_ONLY_KEYS:
                safe[f"{key}_sha256"] = _metadata_hash_value(v)
                safe[f"{key}_redacted"] = True
                continue

            safe[key] = sanitize_system_metadata(
                v,
                key_hint=key,
            )
        return safe
...
DESKTOP_VERIFIER_ID = "methodmesh_audit_v15_2_verification_paths"
METHODMESH_AUDIT_VERSION = "15.2"
...
def console_status(message: str, *, level: str = "INFO") -> None:
    """Timestamped, immediately flushed operational feedback."""
...
class ConsoleProgress:
    ...
    Interactive terminal:
        redraw one progress line.

    Redirected stdout (e.g. > report.txt):
        emit durable progress records at ~5% intervals so the report shows
        where a long run was spending time without producing 300,000 lines.
    """
    ...
    def update(
        ...
        *,
        ...
        if self.tty:
            width = 28
            if self.total > 0:
                done = int(width * min(1.0, self.current / self.total))
                bar = "#" * done + "-" * (width - done)
                text = (
                    f"\r{self.label}: [{bar}] "
                ...
                text = (
                    f"\r{self.label}: {self.current:,} "
                    f"({rate:,.1f}/s)"
                )
            if detail:
                text += f" | {detail}"
            print(text[:220], end="", flush=True)
            return

        # Redirected output: normally write only at 5% boundaries, at the end,
        # or after a meaningful time interval / explicit force.
        if self.total > 0:
            bucket = int((self.current / self.total) * 20)
        else:
            bucket = self.current // 1000

        now = time.monotonic()
        should_log = (
            force
            or self.current == 0
            or (self.total > 0 and self.current >= self.total)
        ...
        console_status(message, level="PROGRESS")

    def finish(self, *, detail: str | None = None) -> None:
        if self.tty:
            self.update(
                self.total if self.total else self.current,
                detail=detail,
                force=True,
            )
            print(flush=True)
...
def collect_central_system_metadata(
    *,
    ...
    requests_to_make = [
        ...
        ("system_assignments", "/v1/assignments", True),
        ("users", "/v1/users", True),
        ("analytics_config", "/v1/config/analytics", False),
        ("analytics_preview", "/v1/analytics/preview", False),
    ]

    progress = ConsoleProgress(
        "Checkpoint: Central metadata endpoints",
        len(requests_to_make) + 1,
    )
    progress.update(0, force=True)

    for index, (name, path, extended) in enumerate(
        requests_to_make,
        start=1,
    ):
        console_status(
            f"Checkpoint metadata: reading {name} ({path})..."
        )
        result = central_optional_json(
            central,
            path,
            extended=extended,
        )
        ...
        if result.get("available"):
            # Sanitize BEFORE this object enters the evidence snapshot.
            # This includes renaming/hashing generic keys such as "value".
            result["data"] = sanitize_system_metadata(
                result.get("data")
            )
        ...
        detail = (
            f"{name}: HTTP {result.get('status_code')}"
            if result.get("status_code") is not None
            else f"{name}: unavailable"
        )
        progress.update(index, detail=detail)

    console_status(
        "Checkpoint metadata: reading server-wide Central audits..."
    )
    ...
    server_audits = endpoint_results["server_audits"]
    if server_audits.get("available"):
        server_audits["data"] = sanitize_system_metadata(
            server_audits.get("data")
        )
    ...
    progress.update(
        ...
        detail=(
            "server_audits: "
            f"{len(server_audits.get('data') or []):,} event(s)"
            if server_audits.get("available")
            else "server_audits: unavailable"
        ),
        force=True,
    )
    progress.finish(detail="system/project metadata collection complete")

    # Users received an additional purpose-built privacy transform in older
    # versions. Preserve it when the API supplied a list; then re-sanitize
    # the resulting structure so generic unsafe keys still cannot survive.
    users = endpoint_results.get("users") or {}
    if users.get("available") and isinstance(users.get("data"), list):
        users["data"] = sanitize_system_metadata(
            sanitize_user_records(users.get("data"))
        )
...
def build_auditor_safe_evidence(
    *,
    ...
    return {
        ...
        "privacy_policy": {
            **(evidence.get("privacy_policy") or {}),
            "research_values_included": False,
            ...
            "cross_project_metadata_included": False,
            "server_audits_project_scoped": True,
            "analytics_other_project_rows_included": False,
            "analytics_system_aggregates_included": True,
        },
        ...
        "central_system": {
            ...
            "scope": central_system.get("scope") or "PROJECT_SAFE",
            "project_id": central_system.get("project_id")
                or evidence.get("project_id"),
            "endpoints":
...
def _dashboard_json_escape(obj) -> str:
    """Escape JSON for safe embedding inside an application/json element."""
    raw = json.dumps(
        obj,
        ensure_ascii=False,
        sort_keys=True,
...
def render_static_auditor_dashboard(
    *,
    evidence: dict,
    checkpoint: dict,
) -> str:
    """Generate a single-file, fully offline auditor/admin dashboard."""
    embedded = _dashboard_json_escape({
        "evidence": evidence,
        "checkpoint": checkpoint,
        "dashboard_version": "methodmesh.dashboard.v1",
    })
...
<title>MethodMesh Audit Dashboard</title>
<style>
:root{color-scheme:light;--bg:#f4f6f8;--panel:#fff;--ink:#17202a;--muted:#667085;--line:#d8dee6;--good:#137333;--warn:#9a6700;--bad:#b42318;--info:#175cd3;--soft-good:#ecfdf3;--soft-warn:#fffaeb;--soft-bad:#fef3f2;--soft-info:#eff8ff;--mono:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono",monospace;--sans:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);font-family:var(--sans);line-height:1.45}
header{background:#111827;color:#fff;padding:28px max(24px,calc((100vw - 1280px)/2))}header .eyebrow{text-transform:uppercase;letter-spacing:.12em;font-size:12px;color:#cbd5e1}header h1{margin:6px 0 4px;font-size:30px}header .sub{color:#cbd5e1;max-width:900px}
main{max-width:1280px;margin:0 auto;padding:24px}nav{display:flex;flex-wrap:wrap;gap:8px;margin:0 0 20px}nav a{text-decoration:none;color:#344054;background:#fff;border:1px solid var(--line);border-radius:999px;padding:8px 12px;font-size:13px}
section{scroll-margin-top:16px;margin:0 0 26px}.section-title{display:flex;align-items:end;justify-content:space-between;gap:16px;margin:0 0 10px}.section-title h2{margin:0;font-size:20px}.section-title p{margin:0;color:var(--muted);font-size:13px}
.grid{display:grid;gap:12px}.grid.cards{grid-template-columns:repeat(auto-fit,minmax(180px,1fr))}.grid.two{grid-template-columns:repeat(auto-fit,minmax(360px,1fr))}
.card,.panel{background:var(--panel);border:1px solid var(--line);border-radius:12px;box-shadow:0 1px 2px rgba(16,24,40,.03)}.card{padding:15px}.card .label{color:var(--muted);font-size:12px;text-transform:uppercase;letter-spacing:.04em}.card .value{font-size:25px;font-weight:650;margin-top:4px;word-break:break-word}.card .note{color:var(--muted);font-size:12px;margin-top:4px}.panel{padding:18px}.panel h3{margin:0 0 10px;font-size:16px}
.badges{display:flex;gap:8px;flex-wrap:wrap;margin-top:14px}.badge{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:6px 10px;font-size:12px;font-weight:650;border:1px solid transparent}.badge.good{color:var(--good);background:var(--soft-good);border-color:#abefc6}.badge.warn{color:var(--warn);background:var(--soft-warn);border-color:#fedf89}.badge.bad{color:var(--bad);background:var(--soft-bad);border-color:#fecdca}.badge.info{color:var(--info);background:var(--soft-info);border-color:#b2ddff}.status-dot{width:8px;height:8px;border-radius:50%;background:currentColor}
.kv{display:grid;grid-template-columns:minmax(150px,34%) 1fr;column-gap:16px;row-gap:8px;font-size:13px}.kv dt{color:var(--muted)}.kv dd{margin:0;overflow-wrap:anywhere}.mono{font-family:var(--mono);font-size:12px;overflow-wrap:anywhere}.muted{color:var(--muted)}
.notice{padding:13px 15px;border-radius:10px;border:1px solid var(--line);margin:10px 0}.notice.good{background:var(--soft-good);border-color:#abefc6}.notice.warn{background:var(--soft-warn);border-color:#fedf89}.notice.info{background:var(--soft-info);border-color:#b2ddff}.notice.bad{background:var(--soft-bad);border-color:#fecdca}
table{width:100%;border-collapse:collapse;font-size:13px}th,td{text-align:left;padding:9px 10px;border-bottom:1px solid #eaecf0;vertical-align:top}th{color:#475467;font-weight:600;background:#f9fafb}tr:last-child td{border-bottom:0}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:10px}
details{border-top:1px solid #eaecf0;padding:10px 0 0;margin-top:10px}details:first-of-type{border-top:0}summary{cursor:pointer;font-weight:600}pre{white-space:pre-wrap;overflow-wrap:anywhere;background:#101828;color:#f8fafc;border-radius:9px;padding:13px;font:12px/1.45 var(--mono)}
.timeline{list-style:none;padding:0;margin:0}.timeline li{display:grid;grid-template-columns:170px 18px 1fr;gap:10px;padding:8px 0;border-bottom:1px solid #eaecf0}.timeline li:last-child{border-bottom:0}.timeline time{color:var(--muted);font:12px var(--mono)}.timeline .glyph{font-weight:800;text-align:center}.timeline .event-title{font-weight:600}.timeline .event-meta{color:var(--muted);font-size:12px}
.split{display:grid;grid-template-columns:minmax(0,1.4fr) minmax(280px,.6fr);gap:12px}button.copy{border:1px solid var(--line);background:#fff;border-radius:7px;padding:5px 8px;cursor:pointer;font-size:12px;float:right}footer{max-width:1280px;margin:0 auto;padding:8px 24px 30px;color:var(--muted);font-size:12px}
@media(max-width:820px){main{padding:16px}.split{grid-template-columns:1fr}.kv{grid-template-columns:1fr}.timeline li{grid-template-columns:1fr;gap:2px}.timeline .glyph{display:none}}
</style>
</head>
<body>
<header>
<div class="eyebrow">MethodMesh · auditor-safe evidence</div>
<h1 id="page-title">Project audit dashboard</h1>
<div class="sub" id="header-sub"></div>
<div class="badges" id="assurance-badges"></div>
</header>
<main>
<nav>
<a href="#overview">Overview</a><a href="#meaning">What this means</a><a href="#system">System</a><a href="#project">Project & forms</a><a href="#timeline">Timeline</a><a href="#exceptions">Exceptions</a><a href="#proofs">Proofs</a><a href="#raw-safe">Safe evidence</a>
</nav>

<section id="overview"><div class="section-title"><h2>Overview</h2><p>Canonical evidence is in AUDIT_EVIDENCE.json; this page is presentation only.</p></div><div class="grid cards" id="overview-cards"></div></section>

<section id="meaning"><div class="section-title"><h2>What this means</h2></div><div class="grid two">
<div class="panel"><h3>What this evidence supports</h3><div class="notice good">MethodMesh cryptographically commits the observed Central state and its privacy-safe evidence. New or changed records are processed only when necessary; unchanged record content is not redownloaded.</div><ul id="supports-list"></ul></div>
<div class="panel"><h3>What it does not prove</h3><ul><li>That clinical or research information entered by a user was factually correct.</li><li>That an account was used by the human to whom it was assigned.</li><li>Continuous service availability outside the recorded probe observations.</li><li>Infrastructure properties that were not exposed to the verifier.</li><li>Device-level collection attestation for ordinary ODK records that lack MethodMesh evidence.</li></ul></div>
</div></section>

<section id="system"><div class="section-title"><h2>Central system</h2><p>Aggregate/system metadata only — no research answers.</p></div><div class="grid cards" id="system-cards"></div><div class="grid two" style="margin-top:12px"><div class="panel"><h3>Service observations</h3><dl class="kv" id="service-kv"></dl></div><div class="panel"><h3>Central metadata coverage</h3><dl class="kv" id="coverage-kv"></dl></div></div><div class="panel" style="margin-top:12px"><h3>Analytics preview</h3><p class="muted">Central's aggregate analytics snapshot. Values shown here are sanitized system/usage metrics from the auditor-safe evidence.</p><div id="analytics-summary"></div><details><summary>Show full sanitized analytics aggregate</summary><pre id="analytics-json"></pre></details></div></section>

<section id="project"><div class="section-title"><h2>Project & forms</h2><p>Metadata, provenance and state; research values are excluded.</p></div><div class="panel"><dl class="kv" id="project-kv"></dl></div><div id="forms-list" class="grid two" style="margin-top:12px"></div><div class="panel" style="margin-top:12px"><h3>Submission provenance</h3><div class="table-wrap"><table><thead><tr><th>Form</th><th>Logical submission</th><th>Status</th><th>Versions</th><th>Last verified</th><th>Assurance</th></tr></thead><tbody id="submission-table"></tbody></table></div></div></section>

<section id="timeline"><div class="section-title"><h2>Evidence timeline</h2><p>● Central-native event · ○ MethodMesh observation · ◆ cryptographic checkpoint</p></div><div class="panel"><ol class="timeline" id="timeline-list"></ol></div></section>

<section id="exceptions"><div class="section-title"><h2>Exceptions & attention</h2></div><div id="exceptions-list"></div></section>

<section id="proofs"><div class="section-title"><h2>Cryptographic proof</h2><p>Independent verification does not require Central credentials.</p></div><div class="split"><div class="panel"><h3>Checkpoint</h3><dl class="kv" id="proof-kv"></dl><div class="notice info" style="margin-top:14px"><strong>SHA-256</strong> asks whether exact bytes changed. <strong>Merkle root</strong> commits the complete evidence leaf set. <strong>RFC3161</strong> provides an independently signed assertion that the checkpoint digest existed no later than the trusted timestamp.</div></div><div class="panel"><h3>Verify</h3><button class="copy" data-copy="python3 verify_bundle.py">Copy</button><pre>python3 verify_bundle.py</pre><details><summary>Manual OpenSSL verification</summary><pre id="openssl-command"></pre></details></div></div></section>

<section id="raw-safe"><div class="section-title"><h2>Safe evidence inventory</h2><p>This is the same sanitized evidence represented elsewhere on this page.</p></div><div class="panel"><details><summary>Show privacy policy</summary><pre id="privacy-json"></pre></details><details><summary>Show Central endpoint availability</summary><div class="table-wrap" style="margin-top:10px"><table><thead><tr><th>Endpoint</th><th>Available</th><th>HTTP</th><th>Elapsed</th></tr></thead><tbody id="endpoint-table"></tbody></table></div></details></div></section>
</main>
<footer>This dashboard is generated from auditor-safe evidence and is not itself a cryptographic authority. Verify the canonical bundle with verify_bundle.py. No network connection is required to view this page.</footer>

<script id="methodmesh-data" type="application/json">""" + embedded + r"""</script>
<script>
"use strict";
const payload=JSON.parse(document.getElementById("methodmesh-data").textContent);
const E=payload.evidence||{},C=payload.checkpoint||{},endpoints=((E.central_system||{}).endpoints||{}),submissions=E.submissions||[];
...
const shortHash=v=>v?String(v).slice(0,16)+"…":"—";
const addKV=(el,key,value,cls="")=>el.insertAdjacentHTML("beforeend",`<dt>${esc(key)}</dt><dd class="${cls}">${esc(fmt(value))}</dd>`);
const badge=(label,cls="info")=>`<span class="badge ${cls}"><span class="status-dot"></span>${esc(label)}</span>`;
const card=(label,value,note="")=>`<div class="card"><div class="label">${esc(label)}</div><div class="value">${esc(fmt(value))}</div>${note?`<div class="note">${esc(note)}</div>`:""}</div>`;
function flatten(obj,prefix="",out={}){if(obj===null||obj===undefined)return out;if(Array.isArray(obj)){obj.forEach((v,i)=>flatten(v,`${prefix}[${i}]`,out));return out}if(typeof obj==="object"){Object.entries(obj).forEach(([k,v])=>{const p=prefix?`${prefix}.${k}`:k;if(v!==null&&typeof v==="object")flatten(v,p,out);else out[p]=v});return out}out[prefix]=obj;return out}

const serverAudits=((endpoints.server_audits||{}).data||[]);
const centralProvenance=(endpoints.server_audits||{}).available===true;
const methodmeshCount=submissions.filter(s=>!!s.original_attestation_hash||!!s.desktop_receipt_sha256||String(s.overall_status||"").includes("DESKTOP_VERIFIED")).length;
const methodmeshVerified=methodmeshCount>0;
$("page-title").textContent=`Project ${E.project_id??"—"} audit dashboard`;
$("header-sub").textContent=`${E.central_url||"ODK Central"} · evidence window ${date(E.window_start_utc)} → ${date(E.window_end_utc)}`;
$("assurance-badges").innerHTML=badge("PROJECT-SCOPED BUNDLE","info")+badge("PLATFORM MONITORED","good")+badge(centralProvenance?"CENTRAL PROVENANCE":"CENTRAL AUDIT LIMITED",centralProvenance?"good":"warn")+(methodmeshVerified?badge("METHODMESH VERIFIED EVIDENCE","good"):badge("NO DEVICE-LEVEL METHODMESH ATTESTATION","info"));

const verifiedPresent=submissions.filter(s=>s.status==="VERIFIED_PRESENT").length;
const deleted=submissions.filter(s=>s.status==="DELETED").length;
const zero=submissions.filter(s=>s.status==="ZERO_VERSION_ANOMALY").length;
const unresolved=submissions.flatMap(s=>s.edit_events||[]).filter(e=>((e.reason||{}).association_status)==="UNRESOLVED").length;
const errors=submissions.filter(s=>s.status==="ERROR"||String(s.overall_status||"").includes("ERROR")).length;
const rfcOK=["VERIFIED","rfc3161_verified"].includes(C.timestamp_status)||["VERIFIED","rfc3161_verified"].includes(C.trusted_timestamp_status);
const auditAttention=errors>0||zero>0||unresolved>0||!centralProvenance;
const findingParts=[];
if(errors)findingParts.push(`${errors} verification error(s)`);
if(zero)findingParts.push(`${zero} zero-version anomaly/anomalies`);
if(unresolved)findingParts.push(`${unresolved} unresolved edit reason(s)`);
if(!centralProvenance)findingParts.push("native Central audit unavailable");
$("overview-cards").innerHTML=[card("Audit findings",auditAttention?"ATTENTION":"NO CURRENT EXCEPTIONS",auditAttention?findingParts.join(" · "):"No current audit exceptions"),card("Checkpoint integrity",rfcOK?"RFC3161 VERIFIED":fmt(C.timestamp_status||C.trusted_timestamp_status),"Cryptographic proof status is separate from audit findings"),card("Verified current",verifiedPresent),card("MethodMesh-evidenced",methodmeshCount,"Records with collection/desktop MethodMesh evidence"),card("Zero-version anomalies",zero),card("Currently deleted",deleted),card("Unresolved edit reasons",unresolved),card("Research values in bundle","NO")].join("");

const supports=["The auditor bundle contains no submission XML, images or attachment bytes.","The current privacy-safe evidence set is committed by a Merkle root and daily checkpoint.","RFC3161 independently timestamps the checkpoint digest.",centralProvenance?"Central-native server audit events are available for actor/time/action provenance.":"Central-native server audit events were not available to this verifier account; MethodMesh observations remain distinguishable from native events.",methodmeshVerified?"At least some records carry MethodMesh collection and/or desktop verification evidence.":"This project is being monitored using Central provenance only; no device-level MethodMesh attestation is claimed."];
$("supports-list").innerHTML=supports.map(x=>`<li>${esc(x)}</li>`).join("");

const service=E.service_summary||{};
addKV($("service-kv"),"Probe observations",service.observation_count);addKV($("service-kv"),"HTTP successes",service.http_success_count);addKV($("service-kv"),"API successes",service.api_success_count);addKV($("service-kv"),"Unexpected open ports",(service.unexpected_open_ports_seen||[]).join(", ")||"none");
const endpointEntries=Object.entries(endpoints);
addKV($("coverage-kv"),"Metadata endpoints available",`${endpointEntries.filter(([,v])=>v&&v.available).length} / ${endpointEntries.length}`);addKV($("coverage-kv"),"Server audit endpoint",centralProvenance?"AVAILABLE":"UNAVAILABLE");addKV($("coverage-kv"),"Project audit events included",serverAudits.length);addKV($("coverage-kv"),"Installation audit events read",(endpoints.server_audits||{}).server_wide_events_read);addKV($("coverage-kv"),"Evidence leaf count",C.leaf_count);

const analytics=((endpoints.analytics_preview||{}).data||{}),af=flatten(analytics);
const patterns=[["Database size",/database.*size/i],["Audit logs",/audit.*(total|logs)/i],["Audit failures",/audit.*fail/i],["Administrators",/(admins|administrators).*total/i],["Archived projects",/archived.*project/i],["Largest form",/(biggest|largest).*form/i],["Client audit attachments",/client.*audit.*attachment/i],["Blob files",/(total_blobs|blob.*total)/i],["Submission backlog",/submission.*backlog/i]];
const metricCards=[],used=new Set();for(const[label,re]of patterns){const hit=Object.entries(af).find(([k,v])=>!used.has(k)&&re.test(k)&&["string","number","boolean"].includes(typeof v));if(hit){used.add(hit[0]);metricCards.push(card(label,hit[1],hit[0]))}}
$("analytics-summary").innerHTML=metricCards.length?`<div class="grid cards">${metricCards.join("")}</div>`:`<div class="notice info">Analytics aggregate is present; expand below to inspect the sanitized structure.</div>`;
$("analytics-json").textContent=JSON.stringify(analytics,null,2);
$("system-cards").innerHTML=[card("Central audit",centralProvenance?"AVAILABLE":"LIMITED"),card("Project audit events",serverAudits.length),card("System endpoints",endpointEntries.filter(([,v])=>v&&v.available).length),card("Analytics preview",(endpoints.analytics_preview||{}).available?"CAPTURED":"UNAVAILABLE")].join("");

addKV($("project-kv"),"Central",E.central_url);addKV($("project-kv"),"Project ID",E.project_id);addKV($("project-kv"),"Bundle scope",((E.central_system||{}).scope)||"PROJECT_SAFE");addKV($("project-kv"),"Cross-project metadata included",yesno((E.privacy_policy||{}).cross_project_metadata_included));addKV($("project-kv"),"Evidence window start",date(E.window_start_utc));addKV($("project-kv"),"Evidence window end",date(E.window_end_utc));addKV($("project-kv"),"Submission state rows",submissions.length);addKV($("project-kv"),"Research values included",yesno((E.privacy_policy||{}).research_values_included));

const formsEndpoint=(endpoints.forms||{}).data||[],formsState=(endpoints.methodmesh_forms_state||{}).data||[],formById=new Map();
[...formsEndpoint,...formsState].forEach(f=>{const id=f.xmlFormId||f.form_id||f.id;if(!id)return;formById.set(String(id),{...(formById.get(String(id))||{}),...f})});
const submissionCounts={};submissions.forEach(s=>{const f=s.form_id||"unknown";submissionCounts[f]=(submissionCounts[f]||0)+1});
$("forms-list").innerHTML=[...formById.entries()].map(([id,f])=>`<div class="panel"><h3>${esc(id)}</h3><dl class="kv"><dt>Name</dt><dd>${esc(f.name||"—")}</dd><dt>Version</dt><dd class="mono">${esc(f.version||"—")}</dd><dt>State</dt><dd>${esc(f.state||(f.deleted_at?"deleted":"—"))}</dd><dt>Submissions represented</dt><dd>${esc(submissionCounts[id]||0)}</dd><dt>Deleted at</dt><dd>${esc(date(f.deletedAt||f.deleted_at))}</dd></dl></div>`).join("")||`<div class="notice info">No form metadata was available in this evidence snapshot.</div>`;
$("submission-table").innerHTML=submissions.map(s=>{const assurance=(!!s.original_attestation_hash||!!s.desktop_receipt_sha256)?"MethodMesh":"Central";return`<tr><td>${esc(s.form_id||"—")}</td><td class="mono">${esc(s.logical_submission_id||"—")}</td><td>${esc(s.status||s.overall_status||"—")}</td><td>${esc(s.version_count_post_run??"—")}</td><td>${esc(date(s.last_verified_at))}</td><td>${esc(assurance)}</td></tr>`}).join("");

const timeline=[];serverAudits.forEach(ev=>timeline.push({time:ev.loggedAt,kind:"central",title:ev.action||"Central audit event",meta:[ev.actorId?`actor ${ev.actorId}`:null,ev.acteeId?`object ${ev.acteeId}`:null].filter(Boolean).join(" · ")}));
((endpoints.methodmesh_event_ledger||{}).data||[]).forEach(ev=>timeline.push({time:ev.loggedAt,kind:"methodmesh",title:ev.action||"MethodMesh observation",meta:ev.acteeId?`object ${ev.acteeId}`:""}));
submissions.forEach(s=>(s.edit_events||[]).forEach(ev=>timeline.push({time:ev.created_at,kind:"edit",title:`Edit observed · ${s.form_id||""}`,meta:`${ev.version_id||""}${((ev.reason||{}).association_status)?" · reason "+(ev.reason||{}).association_status:""}`})));
timeline.push({time:C.trusted_timestamp_time_iso||C.generated_at_utc,kind:"checkpoint",title:"Daily cryptographic checkpoint",meta:`RFC3161 ${C.timestamp_status||C.trusted_timestamp_status||"—"} · ${shortHash(C.daily_checkpoint_sha256)}`});
timeline.sort((a,b)=>new Date(a.time||0)-new Date(b.time||0));const glyph=k=>k==="central"?"●":k==="checkpoint"?"◆":"○";
$("timeline-list").innerHTML=timeline.map(ev=>`<li><time>${esc(date(ev.time))}</time><div class="glyph">${glyph(ev.kind)}</div><div><div class="event-title">${esc(ev.title)}</div><div class="event-meta">${esc(ev.meta||"")}</div></div></li>`).join("")||"<li>No timeline events in this evidence snapshot.</li>";

const exceptions=[];if(zero)exceptions.push({cls:"warn",title:`${zero} zero-version anomal${zero===1?"y":"ies"}`,text:"Central lists the logical submission but MethodMesh has observed zero retained versions. Content verification is not claimed."});if(unresolved)exceptions.push({cls:"warn",title:`${unresolved} unresolved edit reason${unresolved===1?"":"s"}`,text:"A Central comment could not be unambiguously associated with a specific human edit."});if(errors)exceptions.push({cls:"bad",title:`${errors} verification error${errors===1?"":"s"}`,text:"Review the machine evidence and restricted verifier logs before relying on the affected record state."});if(!centralProvenance)exceptions.push({cls:"warn",title:"Central server audit unavailable",text:"Actor/time/action provenance from the native server-wide audit stream was not available to this verifier account."});if(!exceptions.length)exceptions.push({cls:"good",title:"No current audit exceptions detected",text:"No verification errors, zero-version anomalies or unresolved edit reasons are represented in this snapshot."});
$("exceptions-list").innerHTML=exceptions.map(x=>`<div class="notice ${x.cls}"><strong>${esc(x.title)}</strong><br>${esc(x.text)}</div>`).join("");

addKV($("proof-kv"),"Evidence Merkle root",E.evidence_merkle_root,"mono");addKV($("proof-kv"),"Checkpoint SHA-256",C.daily_checkpoint_sha256,"mono");addKV($("proof-kv"),"Previous checkpoint",C.previous_daily_checkpoint_sha256||"GENESIS","mono");addKV($("proof-kv"),"RFC3161 status",C.timestamp_status||C.trusted_timestamp_status);addKV($("proof-kv"),"Trusted timestamp",date(C.trusted_timestamp_time_iso));addKV($("proof-kv"),"TSA",C.trusted_timestamp_authority||"—");
$("openssl-command").textContent=`openssl ts -verify \\
  -in tsa/daily_checkpoint.tsr \\
  -queryfile tsa/daily_checkpoint.tsq \\
  -CAfile trust/cacert.pem \\
  -untrusted trust/tsa.crt

openssl ts -verify \\
  -in tsa/daily_checkpoint.tsr \\
  -digest ${C.daily_checkpoint_sha256||"<checkpoint-sha256>"} \\
  -CAfile trust/cacert.pem \\
  -untrusted trust/tsa.crt`;
$("privacy-json").textContent=JSON.stringify(E.privacy_policy||{},null,2);
$("endpoint-table").innerHTML=endpointEntries.map(([name,v])=>`<tr><td>${esc(name)}</td><td>${esc(yesno(v&&v.available))}</td><td>${esc(v&&v.status_code)}</td><td>${esc(v&&v.elapsed_ms!=null?v.elapsed_ms+" ms":"—")}</td></tr>`).join("");
document.querySelectorAll("button.copy").forEach(btn=>btn.addEventListener("click",async()=>{const value=btn.getAttribute("data-copy")||"";try{await navigator.clipboard.writeText(value);const old=btn.textContent;btn.textContent="Copied";setTimeout(()=>btn.textContent=old,1200)}catch(_){}}));
</script>
</body>
</html>
"""
...
def build_auditor_issues(evidence: dict) -> list[dict]:
    """Translate machine states into plain-English auditor findings."""
    issues = []
    sequence = 0
    ...
    def add_issue(**item):
        nonlocal sequence
        sequence += 1
        item["issue_id"] = f"ISSUE-{sequence:06d}"
        item["research_values_included"] = False
        issues.append(item)

    for submission in evidence.get("submissions") or []:
        record_id = submission.get("logical_submission_id")
        form_id = submission.get("form_id")
        status = submission.get("status")
        overall = str(submission.get("overall_status") or "")

        if status == "ZERO_VERSION_ANOMALY":
            add_issue(
                severity="ATTENTION",
                scope="RECORD",
                category="ZERO_VERSION_ANOMALY",
                title=(
                    "Central lists this record, but no retained version "
                    "is available"
                ),
                record_id=record_id,
                form_id=form_id,
                ...
                what_happened=(
                    "ODK Central lists this logical submission as present, "
                    "but when MethodMesh asked Central for its retained "
                    "version history, Central returned zero versions."
                ),
                why_it_matters=(
                    "MethodMesh has no retained record version from which to "
                    "reconstruct and independently verify the record's "
                    "provenance. This does not show that the research data "
                    "are wrong; it means the provenance chain is incomplete "
                    "for this record."
                ),
                what_to_do=(
                    "Investigate why Central exposes the logical submission "
                    "without a retained version. Until that is explained or "
                    "resolved, do not describe this record as independently "
                    "verified by MethodMesh."
                ),
                evidence_points=[
                    ["Record", record_id],
                    ["Form", form_id],
                    ["Central/ledger status", status],
                    [
                        "Retained versions observed",
                        submission.get("version_count_post_run"),
                    ],
                    ["Created", submission.get("created_at")],
                    ["Updated", submission.get("updated_at")],
                ],
            )

        if status == "DELETED_BEFORE_CONTENT_VERIFICATION":
            add_issue(
                severity="ATTENTION",
                scope="RECORD",
                category="DELETED_BEFORE_CONTENT_VERIFICATION",
                title=(
                    "Record was already deleted before MethodMesh could "
                    "verify its content"
                ),
                record_id=record_id,
                form_id=form_id,
                occurred_at=submission.get("deleted_at"),
                what_happened=(
                    "MethodMesh first encountered this submission in "
                    "Central's deleted inventory. No previously verified "
                    "record content was available in the MethodMesh ledger."
                ),
                why_it_matters=(
                    "The deletion itself can be represented in provenance, "
                    "but MethodMesh cannot make a content-verification claim "
                    "for the pre-deletion record state."
                ),
                what_to_do=(
                    "Review Central's native audit history for the deletion "
                    "and determine whether an independently retained source "
                    "or earlier checkpoint is available."
                ),
                evidence_points=[
                    ["Record", record_id],
                    ["Form", form_id],
                    ["Deleted at", submission.get("deleted_at")],
                    ["Status", status],
                ],
            )
        ...
        if (
            ...
            add_issue(
                ...
                title="MethodMesh could not verify this record successfully",
                record_id=record_id,
                form_id=form_id,
                occurred_at=submission.get("last_verified_at"),
                what_happened=(
                    "The record's latest MethodMesh verification state "
                    "contains a failure or error."
                ),
                why_it_matters=(
                    "A verification failure means the affected provenance "
                    "claim should not be relied upon until the underlying "
                    "cause has been investigated."
                ),
                what_to_do=(
                    "Review the verifier logs and Central history for this "
                    "record, resolve the cause, and run verification again."
                ),
                evidence_points=[
                    ["Record", record_id],
                    ["Form", form_id],
                    ["Status", status],
                    ["Overall verification state", overall],
                    ["Last verified", submission.get("last_verified_at")],
                ],
            )

        for edit in submission.get("edit_events") or []:
            reason = edit.get("reason") or {}
            if reason.get("association_status") != "UNRESOLVED":
                continue
            ...
            changed_fields = [
                ...
                for field in (edit.get("changed_fields") or [])
            ]
            field_text = (
                ", ".join(changed_fields)
                if changed_fields
                else "field names were not available"
            )

            add_issue(
                severity="ATTENTION",
                scope="RECORD_EDIT",
                category="UNRESOLVED_EDIT_REASON",
                title=(
                    "The reason for this edit cannot be proven against "
                    "the specific version"
                ),
                record_id=record_id,
                form_id=form_id,
                occurred_at=edit.get("created_at"),
                version_id=edit.get("version_id"),
                previous_version_id=edit.get("previous_version_id"),
                actor=edit.get("actor"),
                changed_fields=changed_fields,
                what_happened=(
                    "Central records an edit to this submission. MethodMesh "
                    "can show that the edit changed these field(s) by name: "
                    f"{field_text}. However, Central comments do not provide "
                    "a native foreign-key link to a particular submission "
                    "version, so MethodMesh cannot prove which recorded "
                    "reason belongs to this exact edit."
                ),
                why_it_matters=(
                    "The edit itself is represented in the provenance "
                    "history. The unresolved point is the attribution of a "
                    "human reason-for-change to this particular version. If "
                    "your audit policy requires a reason for each edit, this "
                    "remains an open audit finding."
                ),
                what_to_do=(
                    "Review the Central comment and audit history around this "
                    "edit. If the reason can be established, document the "
                    "association explicitly; otherwise retain this as an "
                    "unresolved reason-for-change finding."
                ),
                evidence_points=[
                    ["Record", record_id],
                    ["Form", form_id],
                    ["Edited version", edit.get("version_id")],
                    ["Previous version", edit.get("previous_version_id")],
                    ["Edit time", edit.get("created_at")],
                    ["Actor", edit.get("actor")],
                    ["Changed fields (names only)", field_text],
                    ["Reason association", reason.get("association_status")],
                    [
                        "Why association is unresolved",
                        reason.get("association_basis"),
                    ],
                ],
            )
...
<style>
:root{{--bg:#f4f6f8;--panel:#fff;--ink:#17202a;--muted:#667085;
--line:#d8dee6;--warn:#9a6700;--bad:#b42318;--info:#175cd3;
--soft-warn:#fffaeb;--soft-bad:#fef3f2;--soft-info:#eff8ff;
--mono:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;
--sans:Inter,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}}
*{{box-sizing:border-box}}body{{margin:0;background:var(--bg);color:var(--ink);
font-family:var(--sans);line-height:1.5}}header{{background:#111827;color:white;
padding:24px max(20px,calc((100vw - 980px)/2))}}header a{{color:#bfdbfe}}
main{{max-width:980px;margin:0 auto;padding:24px}}.panel{{background:var(--panel);
border:1px solid var(--line);border-radius:12px;padding:20px;margin-bottom:14px}}
h1{{font-size:27px;margin:7px 0}}h2{{font-size:18px;margin:0 0 9px}}
.eyebrow{{font-size:12px;text-transform:uppercase;letter-spacing:.1em;color:#cbd5e1}}
.badge{{display:inline-block;border-radius:999px;padding:6px 10px;font-weight:700;
font-size:12px;background:var(--soft-warn);color:var(--warn);border:1px solid #fedf89}}
.badge.high{{background:var(--soft-bad);color:var(--bad);border-color:#fecdca}}
.kv{{display:grid;grid-template-columns:minmax(180px,30%) 1fr;gap:8px 14px}}
.kv dt{{color:var(--muted)}}.kv dd{{margin:0;overflow-wrap:anywhere}}
.mono{{font-family:var(--mono);font-size:12px}}.callout{{padding:16px;border-radius:10px;
background:var(--soft-info);border:1px solid #b2ddff}}.action{{padding:16px;
border-radius:10px;background:var(--soft-warn);border:1px solid #fedf89}}
footer{{max-width:980px;margin:0 auto;padding:0 24px 30px;color:var(--muted);
font-size:12px}}@media(max-width:700px){{.kv{{grid-template-columns:1fr}}}}
</style>
</head>
<body>
<header>
<div class="eyebrow">MethodMesh · auditor finding</div>
<div>{breadcrumb}</div>
<h1>{html.escape(title)}</h1>
</header>
<main>{body}</main>
<footer>No research values are shown on this page. The canonical machine evidence
remains in AUDIT_EVIDENCE.json and the cryptographic checkpoint files.</footer>
</body>
</html>'''


def render_issue_detail_page(issue: dict) -> str:
    severity = issue.get("severity") or "ATTENTION"
    badge_class = "high" if severity == "HIGH" else ""

    evidence_rows = []
    for label, value in issue.get("evidence_points") or []:
        if value is None or value == "":
            value = "—"
        evidence_rows.append(
            f"<dt>{html.escape(str(label))}</dt>"
            f"<dd class=\"mono\">{html.escape(str(value))}</dd>"
        )

    body = f'''
<div class="panel">
  <span class="badge {badge_class}">{html.escape(severity)}</span>
  <p><strong>Record:</strong>
  <span class="mono">{html.escape(str(issue.get('record_id') or '—'))}</span></p>
  <p><strong>Form:</strong>
  <span class="mono">{html.escape(str(issue.get('form_id') or '—'))}</span></p>
</div>

<div class="panel">
  <h2>What happened?</h2>
  <p>{html.escape(issue.get('what_happened') or '')}</p>
</div>

<div class="panel">
  <h2>Why does this matter?</h2>
  <div class="callout">{html.escape(issue.get('why_it_matters') or '')}</div>
</div>

<div class="panel">
  <h2>What should the auditor do?</h2>
  <div class="action">{html.escape(issue.get('what_to_do') or '')}</div>
</div>

<div class="panel">
  <h2>Evidence supporting this finding</h2>
  <dl class="kv">{''.join(evidence_rows)}</dl>
</div>

<div class="panel">
  <h2>Privacy boundary</h2>
  <p>This page contains identifiers, provenance states, timestamps and, where
  relevant, <strong>field names only</strong>. It does not contain participant
  answers, old/new field values, submission XML, photographs or attachment
  bytes.</p>
</div>
'''
    return _issue_page_shell(
        title=issue.get("title") or "Audit finding",
        body=body,
        breadcrumb=(
...
def render_issue_index_page(
    ...
    *,
    ...
    for issue in page_issues:
        filename = (
            "issue-"
            + issue["issue_id"].split("-")[-1]
            + ".html"
        )
        rows.append(
            "<tr>"
            f"<td>{html.escape(issue.get('severity') or '')}</td>"
            f"<td><a href=\"{html.escape(filename)}\">"
            f"{html.escape(issue.get('title') or '')}</a></td>"
            f"<td class=\"mono\">{html.escape(str(issue.get('record_id') or '—'))}</td>"
            f"<td class=\"mono\">{html.escape(str(issue.get('form_id') or '—'))}</td>"
            f"<td>{html.escape(str(issue.get('occurred_at') or '—'))}</td>"
            "</tr>"
        )
    ...
    if page_number > 1:
        previous = (
            "index.html"
            if page_number == 2
            else f"page-{page_number - 1:04d}.html"
        )
        navigation.append(f'<a href="{previous}">← Previous</a>')
    if page_number < total_pages:
        navigation.append(
            f'<a href="page-{page_number + 1:04d}.html">Next →</a>'
        )

    body = f'''
<div class="panel">
  <p><strong>{len(issues):,}</strong> current auditor-facing finding(s).</p>
  <p>Click an issue to see a plain-English explanation, why it matters,
  the evidence behind it, and the recommended next action.</p>
</div>
<div class="panel" style="overflow:auto">
<table>
<thead><tr><th>Severity</th><th>Issue</th><th>Record</th><th>Form</th><th>Time</th></tr></thead>
<tbody>{''.join(rows)}</tbody>
</table>
</div>
<div class="panel">{' &nbsp; '.join(navigation) if navigation else 'Page 1 of 1'}</div>
'''
    shell = _issue_page_shell(
        title="Audit findings",
        body=body,
        breadcrumb='<a href="../index.html">Dashboard</a>',
    )
    return shell.replace(
        "</style>",
        "table{width:100%;border-collapse:collapse;font-size:13px}"
        "th,td{text-align:left;padding:9px;border-bottom:1px solid #eaecf0;"
        "vertical-align:top}th{background:#f9fafb}</style>",
    )


def write_auditor_issue_pages(
    *,
    staging: Path,
    evidence: dict,
) -> dict:
    issues = build_auditor_issues(evidence)
    issues_dir = staging / "issues"
    issues_dir.mkdir(parents=True, exist_ok=True)

    (staging / "ISSUES.json").write_text(
        pretty_json(issues),
        encoding="utf-8",
    )
    ...
    for page_number in range(1, total_pages + 1):
        filename = (
            "index.html"
            if page_number == 1
            else f"page-{page_number:04d}.html"
        )
        (issues_dir / filename).write_text(
            render_issue_index_page(
                issues,
    ...
    for issue in issues:
        ...
        (issues_dir / filename).write_text(
            render_issue_detail_page(issue),
            encoding="utf-8",
        )
    ...
    return {
        ...
        "high_count": sum(
            1 for x in issues if x.get("severity") == "HIGH"
        ),
        "attention_count": sum(
            1 for x in issues if x.get("severity") == "ATTENTION"
        ),
        "index_path": "issues/index.html",
        "page_count": total_pages,
    }


def _dashboard_summary_model(
    *,
    evidence: dict,
    checkpoint: dict,
    issue_summary: dict,
    ...
    submissions = evidence.get("submissions") or []
    endpoints = (
        (evidence.get("central_system") or {}).get("endpoints") or {}
    )
    forms = (endpoints.get("forms") or {}).get("data") or []
    server_audits = (
        (endpoints.get("server_audits") or {}).get("data") or []
    )
    ...
    methodmesh_evidenced = sum(
        1
        for item in submissions
        if item.get("original_attestation_hash")
        or item.get("desktop_receipt_sha256")
        or "DESKTOP_VERIFIED" in str(
            item.get("overall_status") or ""
        )
    )

    form_summaries = []
    submission_by_form = Counter(
        str(item.get("form_id") or "unknown")
        for item in submissions
    )
    for form in forms[:25]:
        ...
        form_summaries.append({
            ...
            "submission_state_rows":
                submission_by_form.get(str(form_id), 0),
        })

    top_issues = []
    for issue in (issue_summary.get("issues") or [])[:12]:
        top_issues.append({
    ...
    return {
        "schema": "methodmesh.dashboard_summary.v1",
        "central_url": evidence.get("central_url"),
        "project_id": evidence.get("project_id"),
        "window_start_utc": evidence.get("window_start_utc"),
        "window_end_utc": evidence.get("window_end_utc"),
        "privacy_policy": evidence.get("privacy_policy") or {},
        "central_scope":
            (evidence.get("central_system") or {}).get("scope"),
        "submission_count": len(submissions),
        "status_counts": dict(status_counts),
        "methodmesh_evidenced": methodmesh_evidenced,
        "form_count": len(forms),
        "forms_preview": form_summaries,
        "forms_preview_truncated": len(forms) > len(form_summaries),
        "server_audit_available":
            (endpoints.get("server_audits") or {}).get("available")
            is True,
        "project_audit_event_count": len(server_audits),
        "installation_audit_events_read":
            (endpoints.get("server_audits") or {}).get(
                "server_wide_events_read"
            ),
        "service_summary": evidence.get("service_summary") or {},
        "issue_count": issue_summary.get("issue_count", 0),
        "high_issue_count": issue_summary.get("high_count", 0),
        "attention_issue_count":
            issue_summary.get("attention_count", 0),
        "top_issues": top_issues,
        "checkpoint": {
            "evidence_merkle_root":
                evidence.get("evidence_merkle_root"),
            "daily_checkpoint_sha256":
                checkpoint.get("daily_checkpoint_sha256"),
            "previous_daily_checkpoint_sha256":
                checkpoint.get(
                    "previous_daily_checkpoint_sha256"
                ),
            "timestamp_status":
                checkpoint.get("timestamp_status")
                or checkpoint.get("trusted_timestamp_status"),
            "trusted_timestamp_time_iso":
                checkpoint.get("trusted_timestamp_time_iso"),
            "trusted_timestamp_authority":
                checkpoint.get("trusted_timestamp_authority"),
            "leaf_count": checkpoint.get("leaf_count"),
        },
    }


def render_static_auditor_dashboard_v15(
    *,
    evidence: dict,
    checkpoint: dict,
    issue_summary: dict,
) -> str:
    """Small, guided and exception-first front page."""
    model = _dashboard_summary_model(
        evidence=evidence,
        checkpoint=checkpoint,
        issue_summary=issue_summary,
    )
    embedded = _dashboard_json_escape(model)
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>MethodMesh Auditor Dashboard</title>
<style>
:root{--bg:#f4f6f8;--panel:#fff;--ink:#17202a;--muted:#667085;
--line:#d8dee6;--good:#137333;--warn:#9a6700;--bad:#b42318;
--info:#175cd3;--soft-good:#ecfdf3;--soft-warn:#fffaeb;
--soft-bad:#fef3f2;--soft-info:#eff8ff;--mono:ui-monospace,
SFMono-Regular,Menlo,Monaco,Consolas,monospace;--sans:Inter,system-ui,
-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--ink);
font-family:var(--sans);line-height:1.45}header{background:#111827;color:#fff;
padding:28px max(24px,calc((100vw - 1120px)/2))}header .eyebrow{
text-transform:uppercase;letter-spacing:.12em;font-size:12px;color:#cbd5e1}
header h1{margin:6px 0 4px;font-size:30px}header .sub{color:#cbd5e1}
main{max-width:1120px;margin:0 auto;padding:24px}section{margin:0 0 24px}
.panel,.card{background:var(--panel);border:1px solid var(--line);border-radius:12px;
box-shadow:0 1px 2px rgba(16,24,40,.03)}.panel{padding:18px}.grid{display:grid;
gap:12px}.cards{grid-template-columns:repeat(auto-fit,minmax(175px,1fr))}
.card{padding:15px}.label{color:var(--muted);font-size:12px;text-transform:uppercase;
letter-spacing:.04em}.value{font-size:25px;font-weight:700;margin-top:4px}
.badge{display:inline-block;border-radius:999px;padding:6px 10px;font-size:12px;
font-weight:700;margin:4px 5px 0 0}.good{color:var(--good);background:var(--soft-good);
border:1px solid #abefc6}.warn{color:var(--warn);background:var(--soft-warn);
border:1px solid #fedf89}.bad{color:var(--bad);background:var(--soft-bad);
border:1px solid #fecdca}.info{color:var(--info);background:var(--soft-info);
border:1px solid #b2ddff}.start{border:2px solid #b2ddff;background:#fff}
.start h2{margin-top:0}.steps{display:grid;gap:9px}.step{display:flex;gap:12px;
align-items:center;padding:12px;border:1px solid var(--line);border-radius:9px;
text-decoration:none;color:var(--ink);background:#fff}.step strong{display:block}
.step span{color:var(--muted);font-size:13px}.num{width:30px;height:30px;
border-radius:50%;display:grid;place-items:center;background:#eff8ff;color:var(--info);
font-weight:800;flex:0 0 auto}.issue{display:block;padding:14px;border:1px solid #fedf89;
background:var(--soft-warn);border-radius:10px;margin:8px 0;text-decoration:none;
color:var(--ink)}.issue.high{border-color:#fecdca;background:var(--soft-bad)}
.issue .meta{font-size:12px;color:var(--muted);font-family:var(--mono)}
.issue .arrow{float:right;font-weight:800}h2{font-size:20px;margin:0 0 10px}
.kv{display:grid;grid-template-columns:minmax(180px,32%) 1fr;gap:7px 14px;
font-size:13px}.kv dt{color:var(--muted)}.kv dd{margin:0;overflow-wrap:anywhere}
.mono{font-family:var(--mono);font-size:12px}a.button{display:inline-block;
padding:9px 12px;border-radius:8px;text-decoration:none;font-weight:700;
background:#175cd3;color:white}pre{white-space:pre-wrap;overflow-wrap:anywhere;
background:#101828;color:white;border-radius:9px;padding:12px;font:12px/1.45 var(--mono)}
footer{max-width:1120px;margin:0 auto;padding:0 24px 30px;color:var(--muted);
font-size:12px}@media(max-width:700px){main{padding:16px}.kv{grid-template-columns:1fr}}
</style>
</head>
<body>
<header><div class="eyebrow">MethodMesh · guided auditor view</div>
<h1 id="title">Project audit</h1><div class="sub" id="sub"></div>
<div id="badges"></div></header>
<main>
<section class="panel start">
<h2>Start here</h2>
<p>This page tells you what needs attention before exposing technical detail.</p>
<div class="steps">
<a class="step" href="#issues"><div class="num">1</div><div><strong>Review issues</strong>
<span id="step-issues">Loading…</span></div></a>
<a class="step" href="#proof"><div class="num">2</div><div><strong>Confirm the evidence package verifies</strong>
<span>Check the Merkle root and RFC3161 timestamp.</span></div></a>
<a class="step" href="#scope"><div class="num">3</div><div><strong>Confirm the scope</strong>
<span>Check what this project bundle does and does not contain.</span></div></a>
<a class="step" href="system.html"><div class="num">4</div><div><strong>Review server validation statistics</strong>
<span>Central version, TLS, network ports and aggregate installation health.</span></div></a>
</div>
</section>
<section><div class="grid cards" id="cards"></div></section>
<section id="issues"><div class="panel"><h2>Issues requiring review</h2>
<div id="issue-list"></div><p><a class="button" href="issues/index.html">Review all issues</a></p>
</div></section>
<section id="scope"><div class="panel"><h2>Project scope & privacy</h2>
<dl class="kv" id="scope-kv"></dl><p>This front page intentionally does not
render every submission. Complete machine evidence remains in background files
in this bundle.</p></div></section>
<section><div class="panel"><h2>Forms</h2><div id="forms"></div></div></section>
<section id="proof"><div class="panel"><h2>Cryptographic checkpoint</h2>
<dl class="kv" id="proof-kv"></dl>
<p><span class="badge good">BUNDLE SELF-CHECKED BEFORE PACKAGING</span></p>
<p>The completed ZIP is only produced after MethodMesh's checksum, Merkle,
checkpoint and RFC3161 checks pass. A routine auditor does not need to run a command.</p>
<details><summary>Technical: independently verify this bundle</summary>
<p>An independent technical reviewer can repeat the cryptographic checks locally
without Central credentials.</p>
<p><strong>Where to run it:</strong> first extract the ZIP, then open a terminal
in the extracted bundle root — the same directory that contains
<code>index.html</code>, <code>system.html</code>, and
<code>verify_bundle.py</code>.</p>
<pre>python3 ./verify_bundle.py</pre>
<p>Or, from another working directory, give Python the relative or absolute path
to the extracted bundle:</p>
<pre>python3 ./path/to/extracted-bundle/verify_bundle.py</pre>
<p>The verifier resolves the TSA and trust files relative to its own location,
so it does not matter where your terminal's current working directory is when
you use the second form.</p></details></div></section>
</main>
<footer>No participant answers, old/new research values, XML, photographs or
attachment bytes are rendered by this dashboard.</footer>
<script id="model" type="application/json">''' + embedded + r'''</script>
<script>
"use strict";
...
">":"&gt;",'"':"&quot;","'":"&#39;"}[ch]));
const dt=v=>{if(!v)return"—";const d=new Date(v);return Number.isNaN(d.getTime())?
String(v):d.toLocaleString()};
const kv=(el,k,v,mono=false)=>el.insertAdjacentHTML("beforeend",
`<dt>${esc(k)}</dt><dd class="${mono?"mono":""}">${esc(v??"—")}</dd>`);
$("title").textContent=`Project ${M.project_id} audit`;
$("sub").textContent=`${M.central_url} · ${dt(M.window_start_utc)} → ${dt(M.window_end_utc)}`;
$("badges").innerHTML=`<span class="badge info">PROJECT-SCOPED BUNDLE</span>`+
`<span class="badge good">NO RESEARCH VALUES</span>`+
(M.server_audit_available?`<span class="badge good">CENTRAL PROVENANCE</span>`:
`<span class="badge warn">CENTRAL AUDIT LIMITED</span>`);
const attention=M.issue_count>0;
$("cards").innerHTML=[
`<div class="card"><div class="label">Audit findings</div><div class="value">${attention?"ATTENTION":"NONE"}</div></div>`,
`<div class="card"><div class="label">Current issues</div><div class="value">${M.issue_count}</div></div>`,
`<div class="card"><div class="label">Submission states represented</div><div class="value">${M.submission_count.toLocaleString()}</div></div>`,
`<div class="card"><div class="label">Forms</div><div class="value">${M.form_count.toLocaleString()}</div></div>`,
`<div class="card"><div class="label">MethodMesh-evidenced</div><div class="value">${M.methodmesh_evidenced.toLocaleString()}</div></div>`,
`<div class="card"><div class="label">Checkpoint</div><div class="value">${esc(M.checkpoint.timestamp_status||"—")}</div></div>`
].join("");
$("step-issues").textContent=M.issue_count?
`${M.issue_count} finding(s) currently require review.`:"No current auditor-facing findings.";
$("issue-list").innerHTML=M.top_issues.length?M.top_issues.map(x=>
`<a class="issue ${x.severity==="HIGH"?"high":""}" href="${esc(x.href)}">
<span class="arrow">→</span><strong>${esc(x.title)}</strong>
<div class="meta">${esc(x.record_id||"—")} · ${esc(x.form_id||"—")} · ${esc(dt(x.occurred_at))}</div></a>`
).join(""):`<p><span class="badge good">No current issues</span></p>`;
kv($("scope-kv"),"Bundle scope",M.central_scope||"PROJECT_SAFE");
kv($("scope-kv"),"Project",M.project_id);
kv($("scope-kv"),"Cross-project metadata included",
M.privacy_policy.cross_project_metadata_included===false?"NO":"—");
kv($("scope-kv"),"Research values included",
M.privacy_policy.research_values_included===false?"NO":"—");
kv($("scope-kv"),"Submission XML included",
M.privacy_policy.raw_submission_xml_included===false?"NO":"—");
kv($("scope-kv"),"Attachments/images included",
M.privacy_policy.attachments_included===false?"NO":"—");
$("forms").innerHTML=M.forms_preview.map(f=>
`<div style="padding:9px 0;border-bottom:1px solid #eaecf0">
<strong>${esc(f.form_id||"—")}</strong> · ${esc(f.name||"")}
<div style="font-size:12px;color:#667085">version ${esc(f.version||"—")} ·
state ${esc(f.state||"—")} · ${Number(f.submission_state_rows||0).toLocaleString()}
submission state row(s)</div></div>`).join("")+
(M.forms_preview_truncated?`<p class="mono">Additional forms are present in the
machine evidence but are not rendered on this front page.</p>`:"");
kv($("proof-kv"),"Evidence Merkle root",M.checkpoint.evidence_merkle_root,true);
kv($("proof-kv"),"Checkpoint SHA-256",M.checkpoint.daily_checkpoint_sha256,true);
kv($("proof-kv"),"RFC3161 status",M.checkpoint.timestamp_status);
kv($("proof-kv"),"Trusted timestamp",dt(M.checkpoint.trusted_timestamp_time_iso));
</script>
</body>
</html>'''
...
def build_system_validation_stats(evidence: dict) -> dict:
    endpoints = (
        (evidence.get("central_system") or {}).get("endpoints") or {}
    )
    analytics = (
        (endpoints.get("analytics_preview") or {}).get("data") or {}
    )
    system = analytics.get("system") or {}
    installation = analytics.get("installation_summary") or {}
    probes = evidence.get("service_probes") or []
    probe = probes[-1] if probes else {}
    http = probe.get("http_version_endpoint") or {}
    api = probe.get("authenticated_api") or {}
    dns = probe.get("dns") or {}
    tls = probe.get("tls") or {}
    audit = system.get("num_audit_log_entries") or {}
    admins = system.get("num_admins") or {}

    return {
        "schema": "methodmesh.system_validation_stats.v1",
        "central_url": evidence.get("central_url"),
        "observed_at_utc": probe.get("observed_at_utc"),
        "privacy": {
            "research_values_included": False,
            "project_names_included": False,
            "user_identity_list_included": False,
            "statistics_are_aggregate": True,
        },
        "server": {
            ...
            "http_elapsed_ms": http.get("elapsed_ms"),
            "api_status": api.get("status"),
            "api_code": api.get("status_code"),
            "api_elapsed_ms": api.get("elapsed_ms"),
            "server_header": http.get("server_header"),
            "version_text": http.get("version_text"),
            "version_sha256": http.get("version_text_sha256"),
        },
        "tls": {
            ...
            "protocol": tls.get("protocol") or tls.get("version"),
        },
        "ports": probe.get("tcp_ports") or [],
        "expected_public_ports": probe.get("expected_public_ports") or [],
        "unexpected_open_ports": probe.get("unexpected_open_ports") or [],
        "installation": {
            **installation,
            "database_size_bytes": system.get("database_size"),
            "database_size_human": _human_bytes(system.get("database_size")),
            "num_archived_projects": system.get("num_archived_projects"),
            "num_admins_total": admins.get("total"),
            "num_admins_recent_45d": admins.get("recent"),
            "num_unique_managers": system.get("num_unique_managers"),
            "num_unique_viewers": system.get("num_unique_viewers"),
            "num_unique_collectors": system.get("num_unique_collectors"),
            "num_blob_files": system.get("num_blob_files"),
            "num_blob_files_on_s3": system.get("num_blob_files_on_s3"),
            "num_client_audit_rows": system.get("num_client_audit_rows"),
            "num_questions_biggest_form": system.get("num_questions_biggest_form"),
            "num_xml_only_form_defs": system.get("num_xml_only_form_defs"),
            "uses_external_db": system.get("uses_external_db"),
            "uses_external_blob_store": system.get("uses_external_blob_store"),
            "sso_enabled": system.get("sso_enabled"),
        },
        "audit_health": {
            "entries_total": audit.get("total"),
            "entries_recent_45d": audit.get("recent"),
            "any_failure": system.get("num_audits_failed"),
            "failed_after_retries": system.get("num_audits_failed5"),
            "unprocessed": system.get("num_audits_unprocessed"),
            "client_audit_attachments": system.get("num_client_audit_attachments"),
            "client_audit_attachment_failures":
                system.get("num_client_audit_attachments_failures"),
        },
        "service_history": evidence.get("service_summary") or {},
    }


def render_system_validation_page(stats: dict) -> str:
    embedded = _dashboard_json_escape(stats)
    return r'''<!doctype html>
<html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>MethodMesh System Validation</title>
<style>
body{margin:0;background:#f4f6f8;color:#17202a;font:14px/1.45 system-ui,sans-serif}
header{background:#111827;color:white;padding:24px max(20px,calc((100vw - 1100px)/2))}
header a{color:#bfdbfe}main{max-width:1100px;margin:auto;padding:22px}
.grid{display:grid;gap:12px;grid-template-columns:repeat(auto-fit,minmax(180px,1fr))}
.panel,.card{background:white;border:1px solid #d8dee6;border-radius:12px;padding:16px;margin-bottom:14px}
.label{color:#667085;font-size:12px;text-transform:uppercase}.value{font-size:24px;font-weight:700}
.kv{display:grid;grid-template-columns:minmax(190px,35%) 1fr;gap:7px 14px}
.kv dt{color:#667085}.kv dd{margin:0;overflow-wrap:anywhere}.mono{font:12px ui-monospace,monospace}
table{width:100%;border-collapse:collapse}th,td{text-align:left;padding:8px;border-bottom:1px solid #eaecf0}
th{background:#f9fafb}.note{background:#eff8ff;border:1px solid #b2ddff;border-radius:9px;padding:12px}
pre{white-space:pre-wrap;background:#101828;color:white;padding:12px;border-radius:9px;font:12px/1.4 ui-monospace,monospace}
</style></head><body>
<header><a href="index.html">← Project audit</a><h1>ODK Central system validation</h1>
<p id="sub"></p></header><main>
<div class="note"><strong>Privacy:</strong> aggregate installation statistics and
external service observations only. No participant answers, cross-project record
metadata, project-name list, or user-identity list.</div>
<h2>At a glance</h2><div class="grid" id="cards"></div>
<div class="panel"><h2>Software & API</h2><dl class="kv" id="server"></dl>
<details><summary>Central version detail</summary><pre id="version"></pre></details></div>
<div class="panel"><h2>TLS</h2><dl class="kv" id="tls"></dl></div>
<div class="panel"><h2>Observed network ports</h2><p>Point-in-time reachability from
the verifier host, not a general firewall assessment.</p><table><thead><tr>
<th>Port</th><th>State</th><th>Expected public</th><th>Latency</th></tr></thead>
<tbody id="ports"></tbody></table></div>
<div class="panel"><h2>Installation statistics</h2><dl class="kv" id="inst"></dl></div>
<div class="panel"><h2>Audit-log health</h2><dl class="kv" id="audit"></dl></div>
<div class="panel"><h2>Observed service history</h2><dl class="kv" id="hist"></dl></div>
</main>
<script id="stats" type="application/json">''' + embedded + r'''</script>
<script>
const S=JSON.parse(document.getElementById("stats").textContent),$=x=>document.getElementById(x);
const e=v=>String(v??"—").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]));
const n=v=>v===null||v===undefined?"—":typeof v==="number"?v.toLocaleString():String(v);
const kv=(id,k,v,m=false)=>$(id).insertAdjacentHTML("beforeend",`<dt>${e(k)}</dt><dd class="${m?"mono":""}">${e(n(v))}</dd>`);
$("sub").textContent=`${S.central_url} · observed ${S.observed_at_utc||"—"}`;
const I=S.installation||{},A=S.audit_health||{},V=S.server||{},T=S.tls||{};
$("cards").innerHTML=[
["HTTP",V.http_status],["API",V.api_status],["TLS",T.status],
["Database",I.database_size_human],["Projects",I.num_projects],
["Forms",I.num_forms_total],["Submissions",I.num_submissions_received_total],
["Audit log",A.entries_total]
].map(x=>`<div class="card"><div class="label">${e(x[0])}</div><div class="value">${e(n(x[1]))}</div></div>`).join("");
[["Hostname",V.hostname],["DNS status",V.dns_status],["DNS addresses",(V.dns_addresses||[]).join(", ")],
["HTTP",`${V.http_status||"—"} / ${V.http_code??"—"} / ${V.http_elapsed_ms??"—"} ms`],
["Authenticated API",`${V.api_status||"—"} / ${V.api_code??"—"} / ${V.api_elapsed_ms??"—"} ms`],
["Server header",V.server_header],["Version SHA-256",V.version_sha256]].forEach(x=>kv("server",x[0],x[1],x[0].includes("SHA")));
$("version").textContent=V.version_text||"—";
[["Status",T.status],["Valid from",T.not_before],["Valid until",T.not_after],
["Protocol",T.protocol],["Cipher",T.cipher],["Certificate SHA-256",T.certificate_sha256]].forEach(x=>kv("tls",x[0],x[1],x[0].includes("SHA")));
$("ports").innerHTML=(S.ports||[]).map(p=>`<tr><td>${e(p.port)}</td><td>${e(p.state)}</td><td>${(S.expected_public_ports||[]).includes(p.port)?"YES":"NO"}</td><td>${e(p.elapsed_ms??"—")} ms</td></tr>`).join("");
Object.entries(I).forEach(([k,v])=>kv("inst",k.replaceAll("_"," "),v));
Object.entries(A).forEach(([k,v])=>kv("audit",k.replaceAll("_"," "),v));
Object.entries(S.service_history||{}).forEach(([k,v])=>kv("hist",k.replaceAll("_"," "),Array.isArray(v)?v.join(", "):v));
</script></body></html>'''
...
def safety_scan_auditor_directory(bundle_dir: Path) -> dict:
    ...
    for path in files:
        ...
        dynamic_allowed = (
            rel == "issues/index.html"
            or re.fullmatch(r"issues/page-[0-9]{4}\.html", rel)
            or re.fullmatch(r"issues/issue-[0-9]{6}\.html", rel)
        )

        if rel not in allowed_relative and not dynamic_allowed:
            findings.append(
...
def create_auditor_safe_export(
    *,
    ...
    try:
        ...
        console_status(
            "Auditor export: generating plain-English issue pages..."
        )
        issue_summary = write_auditor_issue_pages(
            staging=staging,
            evidence=safe_evidence,
        )

        console_status(
            "Auditor export: generating installation/server validation page..."
        )
        system_stats = build_system_validation_stats(safe_evidence)
        (staging / "SYSTEM_STATS.json").write_text(
            pretty_json(system_stats),
            encoding="utf-8",
        )
        (staging / "system.html").write_text(
            render_system_validation_page(system_stats),
            encoding="utf-8",
        )

        console_status(
            "Auditor export: generating guided static HTML dashboard..."
        )
        (staging / "index.html").write_text(
            render_static_auditor_dashboard_v15(
                evidence=safe_evidence,
                checkpoint=checkpoint,
                issue_summary=issue_summary,
            ),
...
'''
        ...
        return {
            ...
            "static_dashboard_included": True,
            "dashboard_entrypoint": "index.html",
            "guided_auditor_view": True,
            "issue_count": issue_summary.get("issue_count", 0),
            "issue_index": issue_summary.get("index_path"),
            "system_validation_page": "system.html",
...
def paged_submission_metadata(
    *,
    ...
    console_status(
        f"Reading {kind} submission metadata for form {form_id!r} "
    ...
    while True:
        ...
        result.extend(page)
        console_status(
            f"{form_id}: {kind} metadata page {page_number} "
            f"returned {len(page):,} rows "
            f"({len(result):,} cumulative; {elapsed:.1f}s)",
            level="PROGRESS",
        )
    ...
    console_status(
        f"{form_id}: {kind} metadata complete — {len(result):,} rows."
    )
    return result
...
def infer_central_audit_project_id(event: dict) -> int | None:
    """
    Best-effort project attribution for a Central native audit event.

    Prefer explicit projectId fields from the event/actee/details. If the
    actee itself is a project object, its id is the project ID. Events that
    cannot be attributed safely remain system-scoped (None).
    """
    candidates = []
    ...
    def walk(value):
        if isinstance(value, dict):
            for key, child in value.items():
                ...
                if key_lower in {"projectid", "project_id"}:
                    project_id = _coerce_project_id(child)
    ...
    actee = event.get("actee")
    if isinstance(actee, dict):
        if str(actee.get("type") or "").lower() == "project":
            project_id = _coerce_project_id(actee.get("id"))
            if project_id is not None:
                candidates.append(project_id)
...
def repair_native_event_project_scopes(
    conn: sqlite3.Connection,
) -> dict:
    """
    Repair rows written by pre-14.1 code, which labelled every native
    server-wide audit event with the currently selected project ID.

    MethodMesh synthetic snapshot observations already have intentional
    project scope and are left untouched.
    """
    rows = conn.execute(
        """
        SELECT event_fingerprint, project_id, event_json
        FROM events
        """
    ).fetchall()
    ...
    for row in rows:
        ...
        if event.get("source") == "methodmesh.snapshot_observation":
            continue

        inferred = infer_central_audit_project_id(event)
        if inferred is None:
            system_scoped += 1
        else:
            project_scoped += 1

        if row["project_id"] != inferred:
            conn.execute(
                """
                UPDATE events
                SET project_id = ?
                ...
                """,
                (inferred, row["event_fingerprint"]),
            )
            repaired += 1
    ...
    return {
        ...
        "native_project_scoped": project_scoped,
        "native_system_scoped": system_scoped,
    }


def filter_native_audits_for_project(
    events: list[dict],
    project_id: int,
) -> list[dict]:
    return [
        event
        for event in events
        if infer_central_audit_project_id(event) == int(project_id)
    ]


def _metric(project, *keys):
    value = project
    for key in keys:
...
def scope_analytics_preview_to_project(
    analytics,
    project_id: int,
):
    ...
    result = {}
    if isinstance(analytics.get("system"), dict):
        result["system"] = analytics["system"]

    projects = analytics.get("projects")
    if isinstance(projects, list):
        result["installation_summary"] = {
            "num_projects": len(projects),
            "num_forms_total": sum(
                _metric(p, "forms", "num_forms", "total")
                for p in projects if isinstance(p, dict)
            ),
            "num_forms_recent_45d": sum(
                _metric(p, "forms", "num_forms", "recent")
                for p in projects if isinstance(p, dict)
            ),
            "num_submissions_received_total": sum(
                _metric(p, "submissions", "num_submissions_received", "total")
                for p in projects if isinstance(p, dict)
            ),
            "num_submissions_received_recent_45d": sum(
                _metric(p, "submissions", "num_submissions_received", "recent")
                for p in projects if isinstance(p, dict)
            ),
            "num_submissions_edited_total": sum(
                _metric(p, "submissions", "num_submissions_edited", "total")
                for p in projects if isinstance(p, dict)
            ),
            "num_submissions_with_comments_total": sum(
                _metric(p, "submissions", "num_submissions_with_comments", "total")
                for p in projects if isinstance(p, dict)
            ),
        }
        result["projects"] = [
            ...
            and _coerce_project_id(p.get("id")) == int(project_id)
        ]
...
def scope_central_metadata_for_project(
    central_system: dict,
    project_id: int,
) -> dict:
    """
    Convert an administrator-readable snapshot into project-safe evidence.

    Global endpoint responses that enumerate other projects/users/assignments
    are removed. Safe system aggregates are retained. Native server-wide audit
    events are filtered to events attributable to the selected project.
    """
    source_endpoints = (
        (central_system or {}).get("endpoints") or {}
    )
    ...
    endpoints = {
        name: dict(value)
        for name, value in source_endpoints.items()
        if name in allowed and isinstance(value, dict)
    }

    analytics_endpoint = endpoints.get("analytics_preview")
    if (
        isinstance(analytics_endpoint, dict)
        and analytics_endpoint.get("available")
    ):
        analytics_endpoint["data"] = scope_analytics_preview_to_project(
            analytics_endpoint.get("data"),
            project_id,
        )
        analytics_endpoint["scope"] = (
            "SYSTEM_AGGREGATES_PLUS_SELECTED_PROJECT_ONLY"
        )

    audit_endpoint = endpoints.get("server_audits")
    if (
        isinstance(audit_endpoint, dict)
        and audit_endpoint.get("available")
    ):
        all_events = audit_endpoint.get("data") or []
        selected_events = filter_native_audits_for_project(
            all_events,
            project_id,
        )
        audit_endpoint["server_wide_events_read"] = len(all_events)
        audit_endpoint["project_events_included"] = len(selected_events)
        audit_endpoint["data"] = selected_events
        audit_endpoint["scope"] = "SELECTED_PROJECT_ONLY"

    endpoints["project_scope_policy"] = {
        "available": True,
        "status_code": 200,
        "data": {
            "schema": "methodmesh.project_scope_policy.v1",
            "project_id": int(project_id),
            "cross_project_project_list_included": False,
            "cross_project_user_list_included": False,
            "cross_project_assignments_included": False,
            "cross_project_audit_events_included": False,
            "analytics_system_aggregates_included": True,
            "analytics_other_project_rows_included": False,
        },
    }
    ...
    return {
        "window_start_utc":
            (central_system or {}).get("window_start_utc"),
        "window_end_utc":
            (central_system or {}).get("window_end_utc"),
        "scope": "PROJECT_SAFE",
        "project_id": int(project_id),
        "endpoints": endpoints,
    }
...
def ingest_server_audit_delta(
    ...
    *,
    ...
    inserted = 0
    project_events = 0
    system_or_other_events = 0

    for event in events:
        fp = event_fingerprint(event)
        actual_project_id = infer_central_audit_project_id(event)

        if actual_project_id == int(project_id):
            project_events += 1
        else:
            system_or_other_events += 1

        existed = conn.execute(
            "SELECT 1 FROM events WHERE event_fingerprint = ?",
            (fp,),
        ...
        if (
            ...
            and action in {"submission.delete", "submission.purge"}
        ):
            logical_id = (
    ...
    return {
        ...
        "selected_project_events_read": project_events,
        "other_or_system_events_read": system_or_other_events,
    }
...
def delta_auditor_summary(
    ...
    lines = [
        ...
        f"Currently deleted rows:   {run_stats.get('deleted_observed', 0)}",
        "",
        "LEDGER STATE",
        ...
        "AUDIT FINDINGS",
        (
            "ATTENTION - one or more audit exceptions require review."
            if (
                zero_version > 0
                or unresolved_reasons > 0
                or run_stats.get("verification_errors", 0) > 0
            )
            else "NO CURRENT EXCEPTIONS"
        ),
        (
            "Cryptographic checkpoint status: VERIFIED"
            if checkpoint.get("timestamp_status") == "VERIFIED"
            else "Cryptographic checkpoint status: REVIEW REQUIRED"
        ),
        "These are separate judgements: a valid checkpoint can faithfully",
        "commit evidence that itself contains anomalies or unresolved issues.",
        "",
        "WHAT THIS MEANS",
...
def create_delta_daily_checkpoint(
    *,
    ...
    console_status(
        "Checkpoint: collecting privacy-safe Central project/system metadata..."
    )
    central_system_full = collect_central_system_metadata(
        central=central,
        project_id=project_id,
        form_id=(
    ...
    console_status(
        "Checkpoint: applying project-safe metadata scope "
        "(excluding other projects/users/audit events)..."
    )
    central_system = scope_central_metadata_for_project(
        central_system_full,
        project_id,
    )
    ...
    console_status(
        "Checkpoint: loading non-data submission state from SQLite ledger..."
    )
    ...
    console_status(
        f"Checkpoint: hashing {len(leaves):,} evidence leaves into Merkle tree..."
    )
    ...
    incomplete_marker = daily_dir / "CHECKPOINT_INCOMPLETE.txt"
    incomplete_marker.write_text(
        "This checkpoint run has not completed successfully.\n"
        "Do not treat this directory as a valid daily checkpoint.\n",
        encoding="utf-8",
    )
    ...
    console_status(
        f"Checkpoint: requesting RFC3161 timestamp for "
        f"{checkpoint_sha[:16]}... from {tsa_url}"
    )
    timestamp_result = create_rfc3161_timestamp_for_hash(
        digest_hex=checkpoint_sha,
        tsa_url=tsa_url,
    ...
    console_status(
        "Checkpoint: RFC3161 response received; validating timestamp evidence..."
    )
    checkpoint = {
        **checkpoint_body,
        "daily_checkpoint_sha256": checkpoint_sha,
    ...
    console_status(
        "Checkpoint: building and self-verifying auditor-safe ZIP..."
    )
    ...
    complete_marker = daily_dir / "CHECKPOINT_COMPLETE.txt"
    complete_marker.write_text(
        "MethodMesh daily checkpoint completed successfully.\n"
        f"Checkpoint SHA-256: {checkpoint_sha}\n"
        f"Evidence Merkle root: {root_hash}\n",
        encoding="utf-8",
    )
    try:
    ...
    console_status(
        "Checkpoint: complete — Merkle root and TSA evidence committed."
    )
...
def process_form_delta(
    *,
    ...
    active_progress = ConsoleProgress(
        f"{form_id}: active submissions",
        len(active),
    )
    active_progress.update(0, force=True)

    for active_index, metadata in enumerate(active, start=1):
        logical_id = metadata.get("instanceId")
        if not logical_id:
            active_progress.update(
                active_index,
                detail="row without instanceId skipped",
            )
            continue
        ...
        #
        # Central may still expose a currentVersion identifier in submission
        # metadata even when the retained /versions endpoint returns zero
        # versions. Once MethodMesh has directly observed version_count == 0,
        # that stronger observation wins for classification. Do not redownload
        # content merely to rediscover the same anomaly.
        if (
            prior is not None
            and (
                prior["version_count"] == 0
                or prior["status"] == "ZERO_VERSION_ANOMALY"
            )
        ):
        ...
        if not needs_verify:
            ...
            active_progress.update(
                active_index,
                detail="unchanged; content not downloaded",
            )
            continue

        active_progress.update(
            active_index - 1,
            detail=f"{reason}; verifying content",
            force=True,
        )
        print()
        print(
    ...
    active_progress.finish(
        detail=(
            f"verified={stats['content_verified']}, "
            f"unchanged={stats['content_skipped']}, "
            f"zero-version={stats['zero_version_anomalies']}, "
            f"errors={stats['verification_errors']}"
        )
    )

    deleted_progress = ConsoleProgress(
        f"{form_id}: deleted inventory",
        len(deleted),
    )
    deleted_progress.update(0, force=True)

    for deleted_index, metadata in enumerate(deleted, start=1):
        if not metadata.get("instanceId"):
            deleted_progress.update(
                deleted_index,
                detail="row without instanceId skipped",
            )
            continue
        ...
        deleted_progress.update(deleted_index)

    deleted_progress.finish(
        detail=f"currently deleted={stats['deleted_observed']}"
    )

    if ledger_unresolved_edit_ids(conn, project_id, form_id):
        console_status(
            f"{form_id}: checking Central comments for unresolved edit reasons..."
        )
...
def main():
    ...
    # Make redirected report files update while the process is still running.
    try:
    ...
    console_status(
        f"Starting MethodMesh v{METHODMESH_AUDIT_VERSION} for Central {args.central}, "
        f"project {args.project}"
        + (f", form {args.form}" if args.form else ", all active forms")
    )
    ...
    console_status("Opening non-data MethodMesh state ledger...")
    ...
    marker.write_text(
        f"METHODMESH v{METHODMESH_AUDIT_VERSION} NON-DATA STATE\n"
        "=============================\n"
    ...
    scope_repair = repair_native_event_project_scopes(conn)
    if scope_repair.get("repaired_rows"):
        console_status(
            "Repaired native Central audit event scope for "
            f"{scope_repair['repaired_rows']:,} existing ledger row(s)."
        )
    ...
    console_status(
        "Running point-in-time DNS/TLS/HTTP/API/targeted-port service probe..."
    )
    ...
    console_status("Selecting ephemeral verifier workspace...")
    ...
    console_status(
        "Loading/pinning RFC3161 TSA public trust material..."
    )
    trust = resolve_tsa_trust_material(
        attestation={
    ...
    console_status(
        "Reading Central server-wide audit delta (if permitted)..."
    )
    ...
    console_status("Enumerating active and deleted forms in the project...")
    ...
    console_status(
        f"Project form inventory: {len(active_forms):,} active, "
        f"{len(deleted_forms):,} deleted historical form object(s)."
    )
    observed_at = utc_now_iso()
    ...
    form_stats = []
    forms_progress = ConsoleProgress(
        "Project forms",
        len(active_forms),
    )
    forms_progress.update(0, force=True)

    for form_index, form_meta in enumerate(active_forms, start=1):
        form_id = form_meta.get("xmlFormId")
        if not form_id:
            forms_progress.update(
                form_index,
                detail="form without xmlFormId skipped",
            )
            continue

        forms_progress.update(
            form_index - 1,
            detail=f"processing {form_id}",
            force=True,
        )
        ...
        forms_progress.update(
            form_index,
            detail=f"{form_id} complete",
            force=True,
        )

    forms_progress.finish(detail="all selected forms complete")
    ...
    run_stats = {
        ...
        "server_audit_project_events_read":
            audit_delta.get("selected_project_events_read"),
        "server_audit_other_or_system_events_read":
            audit_delta.get("other_or_system_events_read"),
    ...
    print(
        f"Currently deleted rows:   {run_stats['deleted_observed']}"
    )
    print(
    ...
    print(
        ...
        "(installation-wide source)"
    )
    print(
        "Project-scoped audit events: "
        f"{run_stats.get('server_audit_project_events_read', 0)}"
    )
    ...
    if args.daily_checkpoint:
        console_status(
            "Building project daily evidence checkpoint and requesting RFC3161 timestamp..."
        )
        ...
        if export:
            ...
            if export.get("static_dashboard_included"):
                print("DASHBOARD:        open index.html inside the ZIP")
                print(
                    "AUDITOR ISSUES:   "
                    f"{export.get('issue_count', 0)} "
                    "plain-English finding page(s)"
                )
                print("SYSTEM STATS:     open system.html inside the ZIP")
    ...
    console_status("MethodMesh run completed successfully.")
...
if __name__ == "__main__":
    try:
        ...
        print()
        console_status(
            "Run cancelled by user. No success cursor/checkpoint is advanced "
            "unless that stage had already completed.",
            level="CANCELLED",
        )
        raise SystemExit(130)
    except RuntimeError as exc:
        console_status(str(exc), level="ERROR")
        if "AUDITOR EXPORT SAFETY SCAN FAILED" in str(exc):
            console_status(
                "The auditor ZIP was NOT created. The safety scanner "
                "refused the export rather than packaging ambiguous/raw "
