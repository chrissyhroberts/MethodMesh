...
class ExternalWorkflowActivity : FragmentActivity() {
    ...
    private fun finishWithResult(confirmed: List<ConfirmedWorkflowStep>) {
        ...
        // Browser deep links therefore receive a clipboard result automatically;
        // if the result contains content:// media, BrowserReturnBridge publishes
        // it through MediaStore as well. ODK/Collect result handling is unchanged
        // because this runs only for methodmesh:// browser deep links.
        val browserReturnFields = linkedMapOf<String, Any?>().apply {
            putAll(completeFields)
            put("methodmesh_closeout_status", "completed")
            put("methodmesh_closeout_step_count", confirmed.size.toString())
            put("return_mode", request.returnMode.id)
            put("payload_mode", OutputFormatter.PayloadMode.normalize(payloadMode))
        }
        BrowserReturnBridge.handleIfRequested(
            context = this,
            request = request,
            fields = browserReturnFields,
            browserDeepLink = intent.action == Intent.ACTION_VIEW &&
