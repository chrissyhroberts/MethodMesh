...
object BrowserBiometricCalloutCapabilityScreen : CapabilityScreenSpec {
    ...
    override fun Render(
        ...
        fun recordOutcome(verified: Boolean, authMethod: String, message: String) {
            ...
            if (verified && issuedToken != null) {
                ...
                clipboard.setPrimaryClip(
                    ClipData.newPlainText("MethodMesh biometric verification", issuedToken.token)
                )
                Toast.makeText(
                    androidContext,
                    "Verified. Token copied. Enketo cannot be filled automatically — paste the token into the form.",
                    Toast.LENGTH_LONG
                ).show()
                status = "Biometric confirmed. Verification token copied to the clipboard for manual paste into Enketo."
            } else if (verified) {
        ...
        CapabilityScreenScaffold(
            ...
            Text("On success MethodMesh copies a device-signed verification token to the clipboard, then returns through the external workflow. Stock Enketo cannot accept an Android activity result directly, so the user pastes that token into a normal form field.")
