...
## Source: `RELEASE_NOTES_v2.2.3.md`
...
- The translator supports any ML Kit language pair available on the device.
- Native runs use two independent speaker buttons, so either participant can speak multiple times in a row.
- Each turn captures original speech text, translated text and direction.
- Optional spoken output can read the translation aloud.
...
- The Android app now uses the MethodMesh paper/ink/green palette.
- Home, drawer and capability screens use the new MethodMesh mark and cleaner card surfaces.
- Typography and spacing have been tightened to make the app feel more deliberate and less homemade.
- The adaptive launcher icon has been resized so the mark breathes inside circular Android launchers.
