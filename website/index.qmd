...
MethodMesh is a capability layer that allows XLSForm-based research workflows to access specialist Android functions while keeping the electronic data capture system as the source of the research record.
...
## What MethodMesh does

```text
XLSForm / EDC
    → MethodMesh capability request
    → Android function, sensor, device or application
    → structured result
    → calling form
```

MethodMesh does not aim to replace an EDC system. Instead, it acts as a bridge between research workflows and the rapidly expanding capabilities available on modern Android devices.
...
MethodMesh provides building blocks for:
...
MethodMesh provides a bridge between XLSForm workflows and physical measurement devices.
...
#### AHT20 Temperature/Humidity sensor flashed and provisioned by MethodMesh

![](assets/screenshots/sensor_read1.png){width="250" }  

#### ODK Form collecting data from AHT20 sensor via MethodMesh
...
MethodMesh extends the types of measurements that can be collected through standard field workflows.
...
MethodMesh connects digital records with physical objects.
...
MethodMesh can act as a local workflow engine for devices.
...
MethodMesh can connect research workflows to device communication functions.
...
MethodMesh exposes these capabilities for research workflows, including:
...
MethodMesh connects forms to spatial tools and navigation capabilities.
...
MethodMesh enables field devices to process physical documents and images.
...
Image-output capabilities can return media attachments into ODK/Kobo calls or into MethodMesh output folders. Direct tests now show image previews in the dashboard so it is easier to confirm that the captured or redacted media are what the operator intended.

---

## Android ecosystem integration

MethodMesh can discover and interact with useful capabilities already present in the Android ecosystem.
...
## The MethodMesh principle

MethodMesh is designed as a **Rosetta stone for research applications**:

- EDC systems define the research workflow.
- MethodMesh provides specialist device capabilities.
