---
title: "Chord builder / identifier"
subtitle: "Module source document · Music"
search: true
---

::: {.callout-note}
**Source-local reference.** This document is preserved from `modules/music/docs/README_Chord.md` in the supplied source snapshot. The Master Book remains authoritative for cross-cutting architecture.
:::

**Method ID:** `music.chord`  
...
Build chord tones from root and quality or identify exact chord matches from supplied notes.

## Canonical capability and surfaces

This capability is defined once by `As100ChordMethod`. The module dashboard (where applicable), direct native use, preset execution, protocol execution and ODK/XLSForm are projections over that same method contract. The dashboard is not the implementation boundary. The capability remains independently discoverable through `MusicModule.as100Methods()`, `capabilityScreens()` and `capabilitySettings()`.
...
The primary beef field is `music_chord_result`. All fields below remain contractually available even when the compact native UI does not normally show them:

- `music_chord_result`
- `music_chord_mode`
- `music_chord_symbol`
- `music_chord_notes`
- `music_chord_matches_json`
- `music_chord_status`
- `music_chord_audit_json`
- `music_chord_error`
- `methodmesh_full_json` — transport-level complete payload when requested by ODK/external transport.

`music_chord_audit_json` is the module audit payload (`methodmesh.music.audit.v1`) and remains secondary to the main result.
...
The stable method ID `music.chord` remains an independent protocol step and uses the same declared inputs/outputs. Schedule/widget launches do not create a separate implementation; closeout is delegated to the shared launch-origin contract.
