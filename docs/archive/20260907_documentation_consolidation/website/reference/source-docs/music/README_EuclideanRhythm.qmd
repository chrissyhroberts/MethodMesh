...
::: {.callout-note}
**Source-local reference.** This document is preserved from `modules/music/docs/README_EuclideanRhythm.md` in the supplied source snapshot. The Master Book remains authoritative for cross-cutting architecture.
:::
...
The primary beef field is `music_euclidean_result`. All fields below remain contractually available even when the compact native UI does not normally show them:
...
`music_euclidean_audit_json` is the module audit payload (`methodmesh.music.audit.v1`) and remains secondary to the main result.
...
The stable method ID `music.euclidean_rhythm` remains an independent protocol step and uses the same declared inputs/outputs. Schedule/widget launches do not create a separate implementation; closeout is delegated to the shared launch-origin contract.
