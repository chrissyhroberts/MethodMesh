# MethodMesh aviation v0.3.2 immersive patch

This archive overlays the existing MethodMesh repository with the Development aviation capability pack.

v0.3.2 keeps the v0.3.1 compile fixes and applies the new generic MethodMesh immersive-host contract where it materially improves aviation UX:
...
Two screens opt in locally through `CapabilityHostPresentation.Immersive`:

- `aviation.dashboard`
- `aviation.emergency.instruments`

Both now own a bounded `fillMaxSize()` root, explicit Back/Exit chrome, exactly one finite vertical scroll region, and a fixed snapshot/finish footer. External/ODK calls still auto-return one structured result.

The following remain `CapabilityHostPresentation.Standard` because they are compact form/result workflows:

- `aviation.airfield.nearby`
- `aviation.runway.wind`
- `aviation.altitude.calculate`
- `aviation.e6b.calculate`

No capability ID/module ID special case is added to the MethodMesh host.
