...
## QR scanner crash fix (2026-07-10)

Pinned `androidx.fragment:fragment-ktx:1.8.5` explicitly. The project previously resolved an older transitive `FragmentActivity` implementation which rejected the Activity Result API request code generated by Compose (`Can only use lower 16 bits for requestCode`) before ZXing could open.
