...
This edition also makes preset execution behaviour and the composable scheduler first-class shared contracts. Preset configuration, presentation mode, completion action and returned-data scope are separate concerns. Schedules own **when** an action happens; presets and protocols own **what** happens. Interactive and background execution are both valid where the capability contract supports them, and scheduled interactive work is transient: after completion MethodMesh returns the user to the launch origin rather than claiming the foreground.
...
- [6. Capability module contract](#6-capability-module-contract)
  ...
  - [Version and maturity identity](#version-and-maturity-identity)
...
- [16. Shared device services](#16-shared-device-services)
  ...
  - [Clock assurance](#clock-assurance)
...
- [28. Normative requirement register](#28-normative-requirement-register)
  ...
  - [Preset execution](#preset-execution)
  - [Protocols](#protocols)
...
Presets are saved capability setups over the canonical capability contract. A preset does not define a private implementation of a capability.

A preset stores four distinct kinds of policy:

1. **Configuration** — fixed values plus the explicit set of values to ask for at run time.
2. **Run behaviour** — whether the capability uses its normal presentation, is explicitly shown to the user, or is asked to run in the background.
3. **Completion behaviour** — what should happen after a committed result: return to the launch origin, share, or save.
4. **Returned-data scope** — the useful result only, result plus audit/provenance, or the full structured JSON representation.

These concerns MUST remain separate. Presentation mode is not output mode, and output mode is not persistence policy.

The stored compatibility values are:

- run behaviour: `AUTO`, `INTERACTIVE`, `BACKGROUND`;
- completion: `HOME`, `SHARE`, `SAVE` (`HOME` is the compatibility token for the user-facing **Return** behaviour);
- returned data: `CORE`, `AUDIT`, `FULL`.

The normal UI SHOULD use human language rather than these transport tokens:

- **Auto / Show UI / Background**;
- **Return / Share / Save**;
- **Result / + Audit / + Full JSON**.

`AUTO` is the compatibility-safe default for existing/imported presets that have no stored launch mode. It means “use the capability's normal presentation for this invocation”. It does not mean “always headless”. `BACKGROUND` is an explicit request for unattended execution and is valid only where the capability can complete from supplied/runtime-resolved inputs without operator interaction. `INTERACTIVE` explicitly requests the capability UI.
...
A preset may explicitly opt into a persistent log. This is the user choice made through preset authoring; capabilities remain transient by default and must not silently persist their outputs.

When enabled, the preset owns a named log bundle in Files. Each confirmed invocation receives a stable entry UUID and is recorded in three layers:
...
Repeated delivery of the same execution result is idempotent. Media-copy failures are recorded with the entry and must not be hidden. A log bundle is exportable and shareable as one package containing the summary, JSONL data and media.
...
docs; examples; dependencies; icon hint; version and maturity/connectivity metadata;
ODK integration-card descriptors; tests where possible.

## Version and maturity identity

Every admitted module exposes a nonblank **module version** and every independently callable capability exposes a nonblank **capability version**. Capability version is the authoritative reproducibility identifier for the implementation contract that produced an execution. Module version identifies the packaged module release/context visible to the operator.

A module SHOULD declare its own semantic release version when it has an independent module release lifecycle. During migration of older modules that only version their contained capabilities, shared metadata infrastructure MAY derive the displayed/audited module version deterministically from the highest contained capability version. Such derivation is a compatibility rule, not evidence that all contained capabilities have identical versions. Capability version remains authoritative for an individual execution.

Version and maturity have one source of truth. Shared Home/Capabilities/Workbench surfaces, capability headers and the execution-envelope builder MUST consume the same canonical metadata rather than maintaining capability-specific version/status tables.

When an execution completes, MethodMesh freezes the producing module/capability identity, version and maturity into the `ExecutionResult`. A later application/module upgrade MUST NOT cause regenerated/exported historical sidecars to claim that the newer capability version produced the older execution.
...
exactly one **maturity state**:

- `Production` (`production` on the execution-envelope wire)
- `Development` (`development` on the execution-envelope wire)
- `Experimental` (`experimental` on the execution-envelope wire)

Module/capability source metadata MUST use the canonical human labels exactly; legacy aliases such as `Stable`, `Preview`, `Prototype` or arbitrary prose are not valid maturity declarations. Shared code represents maturity as a typed value rather than parsing arbitrary UI strings.

These values are mutually exclusive. A multi-capability module may contain
capabilities at different maturity levels. A module MAY explicitly override its
module-level maturity summary/default. During migration of older modules that
do not yet declare that summary, shared metadata infrastructure derives it
conservatively from contained capability states: all Production -> Production,
all Experimental -> Experimental, otherwise Development. A capability with an
explicit maturity remains authoritative for that individual method; a capability
without an explicit state inherits the module/default state. The derived summary
MUST NOT silently promote an explicitly Development or Experimental capability.
...
Preset authoring is a polished projection of the same capability configuration, not a second inferior text-box implementation.
...
name + optional description
    ↓
choose Fixed versus Ask when run
    ↓
choose run behaviour
    ↓
choose completion behaviour
    ↓
choose returned-data scope
    ↓
...
- **Logging** — explicit opt-in only.

Do not require a second naming dialog after the configuration is complete.

The raw saved-settings JSON is an advanced inspection surface, not the primary authoring UI. It may be available behind an Advanced/Technical details disclosure.
...
Preset returned-data preferences do not weaken the ODK transport invariant: every handled MethodMesh -> ODK roundtrip still returns/captures `methodmesh_full_json` as required by the ODK contract.

Where capability metadata can establish that unattended execution is impossible, the authoring UI SHOULD disable Background and explain why. Until such metadata is universally available, Background remains an explicit advanced choice and the canonical capability must fail clearly rather than disappearing or pretending to have completed.
...
Every generated FULL envelope contains the shared versioned `time_assurance`
object described in the Output Contract. That object is injected centrally and
is not a capability-owned output field. Native runtime may keep the FULL JSON
secondary/optional; ODK always receives the metadata projection as part of a
successful/handled roundtrip.

The universal `time_assurance` object is policy-neutral evidence. Canonical
XLSForms MUST NOT duplicate the complete object into a synthetic family of
flat clock fields. A capability or governed workflow whose result genuinely
depends on time MAY additionally expose a small number of useful flat temporal
results, but its explicit temporal policy/decision remains separately recorded
from the underlying Clock Assurance evidence.
...
capability contracts are heterogeneous in those fields. Universal temporal
provenance is the deliberate exception: it is defined centrally inside the
canonical FULL envelope as `time_assurance`, not as invented XLSForm leaves.
Any other future universal success/capability-payload envelope must likewise be
defined centrally and projected consistently; it must not emerge as an
XLSForm-only convention.
...
Every generated FULL JSON payload MUST contain the universal versioned
...
ODK does not need synthetic flat `module_version`/`capability_version` leaves on every form. The canonical values travel in `methodmesh_full_json`; a capability MAY expose a convenient flat version field only where a workflow has a genuine need for it.

## Universal temporal provenance

Temporal provenance is part of the canonical MethodMesh execution evidence.
Every generated FULL execution envelope MUST contain a top-level versioned
`time_assurance` object describing the clock evidence available when that
execution was finalised/returned. This applies whether or not time affects the
scientific or operational meaning of the capability result.

The universal object is **policy-neutral evidence**. It MUST NOT call an
implicit capability policy merely to label an ordinary execution as acceptable
...
The exact field set is owned by shared Clock Assurance/output infrastructure
and evolves only through an explicit `time_assurance.schema_version`. The
object SHOULD retain the raw evidence required to understand the observation:
observed wall time; monotonic reading and boot/session continuity; trusted-time
estimate/bounds where available; uncertainty; anchor source/evidence
commitment/age where available; wall-clock comparison/anomaly state; and an
explanatory reason/status.

Absence of trusted time MUST be explicit rather than represented by omission.
For example, a device with no usable anchor may report
`evidence_state=wall_clock_only`; after a reboot that breaks monotonic
continuity it may report `reboot_unanchored`; corrupt/unusable anchor state may
be represented as indeterminate. `trusted_time` and `anchor` may then be null,
but `observed_wall_time_iso` and the reason for reduced assurance remain part
of the evidence.

Wall-clock anomaly and trusted-time evidence remain separate dimensions. A
rollback or forward divergence can be reported even where a same-boot trusted
interval remains available.

The sidecar is historical evidence. Its `time_assurance` object MUST describe
what MethodMesh knew when that execution occurred. It MUST NOT later be
silently regenerated or upgraded from a newer anchor. Sentinel or another
verifier may evaluate the retained evidence later, but the original envelope
remains unchanged.

### Time-dependent decisions

A capability or governed workflow whose result materially depends on time MUST
apply an explicit temporal policy and record that policy/decision separately
from universal evidence. A representative structure is:
...
`time_assurance` states **what temporal evidence existed**.
`time_decision` states **what a particular capability/workflow did with that
evidence**. Ordinary capabilities that do not use time semantically SHOULD omit
`time_decision` rather than inheriting a synthetic policy decision.

ODK canonical examples continue to capture `methodmesh_full_json` rather than a
large universal set of hidden clock fields. Where a workflow genuinely needs a
convenient scalar temporal return, the capability MAY expose declared fields
such as `clock_assurance_status`, `credential_time_status` or
`decision_time_iso`; those convenience fields do not replace the complete FULL
evidence.
...
A single manually run preset invokes the same canonical capability contract without protocol theatre.

Preset launch mode controls presentation:

- **Auto** uses the capability's normal presentation for that invocation;
- **Show UI** explicitly opens the capability's interactive surface;
- **Background** requests unattended execution without opening the capability UI.

Fixed preset settings remain hidden. Runtime inputs and operational controls appear where required. Interactive runs show current outputs in-place and retain the ordinary live-result -> Commit lifecycle.

Background execution is not a second implementation. It invokes the same capability method with the same settings/runtime contract and is appropriate only when the capability can finish without operator interaction.

Commit freezes the execution result and applies the preset's completion action and returned-data scope. User-facing completion language is **Return / Share / Save**. Return means hand control back to the launch origin; it does not mean “always open MethodMesh Home”.

Do not insert a generic result-only page merely because the launch came from a preset, and do not force every preset into headless execution merely because a caller is automated.
...
Schedules own **when** work happens. Presets and protocols own **what** happens.

New schedule authoring targets a preset action or a notifier action rather than duplicating capability-specific execution logic inside the scheduler. Existing legacy `ResearchSchedule` records may remain readable/editable for compatibility, but new schedule creation uses the composable schedule-plan model.

Current first-class schedule patterns are:

- **Once** — one occurrence now or at a selected date/time;
- **Interval** — every N minutes/hours/days from the activation anchor;
- **Daily** — at one or more local wall-clock times every day;
- **Weekly** — selected weekdays/times;
- **Monthly** — selected calendar-day/monthly rules;
- **Day sequence** — relative Day 1 / Day 2 / Day N patterns from an activation anchor;
- **Cron** — advanced calendar recurrence for cases that are awkward to express through the normal builders.

Cron is an advanced calendar representation, not required user vocabulary.

### Interval versus calendar recurrence

Elapsed intervals and calendar schedules are intentionally different.

If an hourly interval is started at 16:23 with “run immediately” enabled, occurrences are anchored to that trigger:

```text
16:23 now -> 17:23 -> 18:23 -> 19:23 ...
```

An interval is therefore based on elapsed duration from the activation anchor.

A Daily schedule started at 16:23 may run immediately and then recur at 16:23 **local wall-clock time** on following days. Daily/weekly/monthly rules preserve the intended local clock time across timezone/DST changes; they are not defined as repeated 24-hour durations.

### Start and stop policy

Schedule activation is an explicit part of the plan. Supported launch semantics include manual trigger/Day 1, start now and absolute date/time start.

Where a recurrence is anchored to the actual trigger time, “run immediately when started” is a first-class option. It means occurrence zero is due at activation and later occurrences are calculated from that same anchor.

Schedules must also know when to stop. Supported termination policies include a finite duration, a finite occurrence count and **Forever** where indefinite operation is genuinely intended.

### Scheduled preset presentation

A scheduled preset action has a presentation policy:

- **Follow preset** — default; respect the preset's Auto / Show UI / Background launch mode;
- **Show UI** — explicitly override this scheduled occurrence to interactive presentation;
- **Background** — explicitly request unattended execution for this scheduled occurrence.

The scheduler MUST NOT force every preset headless. A Weather dashboard, camera, picker or other operator-facing preset may need to open visibly; a supplied-input weather capture or other automation-capable preset may deliberately run in the background.

The override changes presentation only. It does not create a private copy of the capability and does not change the preset's returned-data or completion contract.

### Notifications and exact timing

A due interactive/notifier schedule may post a high-importance reminder to the notification shade and, subject to Android/user privacy settings, the lock screen. The reminder may carry a schedule-defined human message and a Run action.

Where exact user-requested timing is required on Android versions that gate exact alarms, MethodMesh declares/requests the platform **Alarms & reminders** access and re-arms schedules when that access becomes available. If exact-alarm access is unavailable, the UI must not silently imply second-perfect timing while Android is batching an inexact alarm.

### Modest transient launch

A notification-triggered interactive schedule is a transient interruption, not a request to take over the user's task stack.

Scheduled interactive work should run in a temporary task excluded from Recents where practical. On completion/cancel, that transient task is removed so Android reveals what the user was doing underneath:

```text
desktop -> scheduled preset -> finish -> desktop
other app -> scheduled preset -> finish -> other app
MethodMesh -> scheduled preset -> finish -> MethodMesh
```

The scheduler engine records/arms the next occurrence independently of this UI closeout.

Schedule widgets toggle/trigger schedules without becoming a second scheduler implementation.

## Scheduler shared-runtime contract

The canonical scheduler is a shared orchestration service over canonical targets.

Its timing primitives are:

- one-shot instant;
- elapsed interval;
- local-calendar recurrence (Daily/Weekly/Monthly and advanced Cron);
- relative day sequence.

Schedule plans persist timezone, activation/start policy, termination policy, lanes/actions and recurrence rules. Long/forever schedules may materialise future occurrences in rolling windows rather than constructing an unbounded alarm list.

A preset target is identified by preset ID and is resolved through `ProtocolLibraryRepository`; the scheduler dispatches the canonical preset rather than cloning its settings/implementation. A notifier action contains only notification behaviour.

Schedules are local to the device unless an explicit export/import or remote orchestration contract says otherwise. Export/import remains deterministic and integrity-checked where supported.

The architecture rule is now normative:

> **Presets/protocols own what. Scheduler owns when.**

Legacy scheduler target fields for direct ODK/web-form scheduling are compatibility artefacts, not the model for new schedule authoring. External forms/web actions should be represented by the preset/capability that already owns that action, then scheduled by reference.
...
A preset carries orchestration metadata around the canonical method: configuration/runtime-input policy, run behaviour, completion action, returned-data scope and optional logging. Those dimensions remain distinct and travel with the preset when it is launched directly, from a protocol or from a schedule.

Protocol steps SHOULD normally follow the preset's run behaviour. An orchestration surface may explicitly override presentation where its contract permits, but that override must not mutate the saved preset or create a private capability implementation.

Presentation and data representation are independent. “Background” does not imply “Full JSON”; “Show UI” does not imply “Result only”.

A protocol-pipes layer may bind outputs from one step to inputs of another. The semantic model is defined before any visual editor.
...
MethodMesh has one shared **Clock Assurance** service for capabilities that make security, eligibility, validity, expiry or audit decisions based on time. Capability modules MUST NOT independently promote `Instant.now()`, `System.currentTimeMillis()` or another device wall-clock reading into trusted current time.

The same core service also supplies the policy-neutral temporal evidence snapshot embedded in every canonical FULL execution envelope. Universal evidence capture does not imply that the capability has made a time-sensitive decision and MUST NOT introduce a hidden default freshness/acceptance policy. Policy-dependent states such as whether an anchor is fresh enough for credential expiry, protocol eligibility or another governed action belong to an explicit `time_decision`/workflow evaluation.

The service keeps three concepts separate:

- **wall time** — the Android wall-clock observation, useful but user/network adjustable and therefore not inherently trusted;
- **monotonic elapsed time** — Android elapsed realtime, suitable for measuring elapsed duration only while the same boot session can be established;
- **trusted time anchor** — externally validated time evidence bound to a bounded monotonic acquisition window and retained with provenance.

A trusted anchor records the authority/source, a SHA-256 commitment to the validating evidence, the trusted UTC instant, the monotonic acquisition window, source precision and a boot-session discriminator. The persisted anchor lives in the app's non-backed-up storage and is authenticated with an Android-Keystore HMAC. An unreadable or unauthenticated persisted anchor is **indeterminate** and MUST NOT be silently trusted. A newly validated trusted anchor MAY replace such a damaged prior anchor because the new external evidence is independently authoritative.

Trusted current time is represented as an **interval**, not false point precision. Initial uncertainty includes at least half the external acquisition window plus declared source precision; uncertainty then grows with monotonic oscillator drift. The service exposes the midpoint estimate together with lower and upper trusted bounds. A capability evaluating an expiry/validity boundary MUST treat overlap between those bounds and the boundary as indeterminate/overlapping rather than manufacture a precise before/after answer.

Clock assurance and wall-clock anomaly are separate axes. While same-boot monotonic continuity survives, a trusted current-time interval may remain usable even if the Android wall clock has been moved backwards or forwards. The service therefore reports both an assurance state and an independent wall-clock state.

Canonical assurance states are:

- `trusted_fresh` — trusted monotonic estimate exists and is within the caller's declared age/uncertainty policy;
- `trusted_aged` — the trusted monotonic estimate still exists, but age or accumulated uncertainty exceeds the caller's preferred threshold;
- `wall_clock_only` — no trusted anchor exists;
- `reboot_unanchored` — a prior anchor exists but same-boot monotonic continuity can no longer be established;
- `indeterminate` — stored assurance evidence is unreadable/unauthenticated or otherwise cannot be interpreted safely.

Canonical wall-clock states are `consistent`, `diverged_forward`, `rollback_detected` and `not_comparable`. Divergence is measured against the trusted interval, not merely its midpoint.

There is deliberately no universal freshness threshold. A capability whose correctness depends on time declares the age, uncertainty and divergence policy appropriate to that operation. Shared code may provide named reusable policy presets, but no implicit default may silently determine whether `trusted_aged`, `wall_clock_only` or another state is acceptable.

Reboot is a hard assurance boundary. Monotonic elapsed time is not UTC and MUST NOT be extrapolated across a reboot merely because a new elapsed value happens to be larger than an old one. Clock Assurance uses a boot-session identity (Android boot count with a kernel boot-ID fallback) and degrades to `reboot_unanchored` until new trusted evidence establishes a fresh anchor.
...
### Workbench and Home Clock Assurance surfaces
...
Trusted Timestamp and Clock Assurance are distinct. Trusted Timestamp creates and validates portable RFC 3161 evidence about when exact bytes existed. Clock Assurance may consume suitable externally validated time evidence to maintain a reusable current-time estimate. Neither substitutes for the other.

An RFC 3161 result MUST NOT become a shared clock anchor merely because its CMS signature is internally consistent. For a configured trusted authority, the validating path must bind the token signer to the configured/pinned signer material, validate the configured certificate chain and certificate validity at the token generation time, require the critical RFC 3161 `id-kp-timeStamping` EKU, and retain a SHA-256 commitment to the timestamp evidence. A custom/unconfigured TSA may still produce portable proof according to its capability contract, but it MUST NOT silently become a trusted Clock Assurance source.

The current Trusted Timestamp module may refresh Clock Assurance only when its configured trust evaluation returns `trusted_registry_match`. The acquisition is bracketed by monotonic snapshots so network/certificate-fetch latency expands uncertainty instead of being mistaken for exact local time.

Attestation timestamp evidence is not automatically a Clock Assurance anchor. Any attestation or future source that wishes to publish a clock anchor must pass the same shared externally-validated-anchor contract rather than relying on a weaker module-local timestamp check.
...
Verify that intentional fixed settings persist; fixed settings do not reappear as runtime questions; runtime inputs remain runtime; and one-off run text is not accidentally saved.
...
- interactive preset launch uses the same canonical capability UI;
- completion action is Return / Share / Save and returns to the correct launch origin;
- returned-data scope is Result / + Audit / + Full JSON and does not get conflated with presentation mode;
- optional persistent logging is opt-in and does not become implicit capability storage.

Preset authoring should be coherent, progressively disclosed and capability-aware. Raw JSON is an advanced inspection view, not the primary setup interface.
...
Verify schedule timing semantics rather than only checking that an alarm eventually fires: elapsed intervals are anchored to the actual start trigger; Daily/Weekly/Monthly preserve local wall-clock intent; immediate-first-run behaviour is explicit; and finite/forever end policies behave as authored.

Scheduled preset execution follows the preset's launch mode by default and may explicitly override to Show UI or Background. Universal forced-headless execution is a defect.

Where exact timing is promised, verify Android exact-alarm access/permission handling and re-arming. Verify lock-screen/shade notifications subject to platform settings.

Widget completion normally returns to Android desktop. A notification-launched interactive scheduled task is transient and, after completion/cancel, reveals the app/desktop that was underneath rather than routing gratuitously to MethodMesh Home. Background scheduled work records completion and arms the next occurrence without showing unnecessary UI.
...
For Clock Assurance changes, test at minimum: no-anchor wall-clock-only state; trusted-anchor monotonic advancement; acquisition-window uncertainty; oscillator-drift growth; wall-clock rollback and forward divergence without destroying a valid monotonic trusted timeline; reboot/boot-ID discontinuity even when the new elapsed-realtime value is numerically larger; validity-boundary overlap; rejection of same-boot trusted-time regression; unauthenticated/corrupt persisted anchor handling; safe replacement by fresh externally validated evidence; and refusal to publish anchors from unconfigured or incompletely validated time authorities.
...
15. Every admitted module/capability resolves to exactly one canonical maturity state (`Production`, `Development`, `Experimental`) and exactly one connectivity tag (`ONLINE_ONLY`, `OFFLINE`, `ONLINE_OFFLINE`). Do not infer these from location or UI surface.
...
- **`MM-ODK-006`** - `methodmesh_full_json` is the shared complete structured/audit return for ODK calls and remains available independently of any capability-owned `*_json` field; every generated FULL envelope includes centrally defined versioned `time_assurance` evidence rather than XLSForm-invented universal clock fields.
...
- **`MM-OUT-002`** - `methodmesh_full_json` remains available as the shared complete structured/audit projection, including for ODK capability calls even when no capability-owned JSON field exists, and every generated FULL envelope contains versioned universal `time_assurance` evidence.
...
- **`MM-OUT-010`** - Universal `time_assurance` is injected by shared execution/output infrastructure for every generated FULL envelope; capability modules and XLSForms MUST NOT independently reconstruct or shadow it.
- **`MM-OUT-011`** - Universal `time_assurance` is policy-neutral evidence. A separate `time_decision` is recorded only where a capability/workflow materially depends on time and applies an explicit policy.
- **`MM-OUT-012`** - The `time_assurance` object is historical execution evidence and MUST NOT be silently recomputed or upgraded from anchors obtained after the execution.
- **`MM-OUT-013`** - Every generated canonical FULL envelope records the frozen producing module/capability identity, version and maturity. Software provenance is captured at execution completion and MUST NOT be re-resolved from newer registry metadata when historical results are exported later.
- **`MM-OUT-014`** - Capability version is mandatory reproducibility provenance. Multi-capability combined workflows retain each distinct producing capability identity/version in the canonical envelope.
...
- **`MM-CLOCK-002`** - Trusted current time is a bounded interval derived from externally validated evidence plus same-boot monotonic elapsed time; initial acquisition uncertainty and accumulated oscillator drift remain explicit.
- **`MM-CLOCK-003`** - Reboot/boot-session discontinuity breaks monotonic trust continuity. Old elapsed realtime MUST NOT be bridged to a new boot by numeric comparison alone.
- **`MM-CLOCK-004`** - Clock assurance state and wall-clock anomaly state are independent; wall-clock rollback/divergence does not erase an otherwise valid same-boot trusted interval.
- **`MM-CLOCK-005`** - Persisted clock anchors are non-backed-up, integrity/authenticity protected, and fail closed when unreadable or unauthenticated; fresh externally validated evidence may establish a replacement anchor.
- **`MM-CLOCK-006`** - Externally supplied time becomes a trusted anchor only after the producing source's configured trust contract is satisfied. Cryptographic self-consistency or an unconfigured TSA is insufficient.
- **`MM-CLOCK-007`** - Capabilities declare the freshness/uncertainty policy appropriate to their own decision and return/retain material assurance evidence when it changes interpretation; shared infrastructure does not manufacture a universal Boolean validity answer.
- **`MM-CLOCK-008`** - A time-sensitive decision whose trusted uncertainty interval overlaps its validity boundary is reported as overlapping/indeterminate rather than coerced to before/within/after by midpoint alone.
- **`MM-CLOCK-009`** - Every generated canonical FULL execution envelope carries a versioned, policy-neutral `time_assurance` snapshot from shared core infrastructure, regardless of whether the capability uses time semantically.
- **`MM-CLOCK-010`** - Universal `time_assurance` records raw temporal evidence (wall observation, monotonic/boot continuity, trusted bounds/uncertainty when available, anchor provenance/age, wall-clock comparison and explanatory state) rather than manufacturing a universal accept/reject decision.
- **`MM-CLOCK-011`** - Missing, stale, reboot-broken, corrupt or otherwise insufficient trusted-time evidence is represented explicitly in the sidecar; absence of assurance MUST NOT be represented by silently omitting temporal provenance or promoting wall time to trusted time.
- **`MM-CLOCK-012`** - Policy-dependent labels and decisions belong to the explicit capability/workflow temporal policy. Methods whose outcome materially depends on time record that evaluation separately as `time_decision` or an equivalent versioned decision object.
...
- **`MM-PRESET-001`** - A preset stores configuration/runtime-input policy, run behaviour, completion action and returned-data scope as distinct first-class concerns over the canonical capability.
...
- **`MM-SCHED-002`** - Presets/protocols own what; the scheduler owns when and references canonical targets rather than cloning their implementation.
...
- **`MM-MOD-004`** - Every admitted module exposes a nonblank module version and every independently callable capability exposes a nonblank capability version; capability version is authoritative for reproducibility of an individual execution.
- **`MM-MOD-005`** - Maturity is a typed canonical value (`Production`, `Development`, `Experimental`), not arbitrary prose; shared UI and execution-envelope serialization consume the same canonical metadata and MUST NOT maintain capability-specific status tables.
- **`MM-MOD-006`** - Until a legacy module owns independent semantic release versioning, shared infrastructure MAY deterministically derive its module display/audit version from contained capability versions; this compatibility rule does not replace the authoritative per-capability version.
...
- Added Workbench as the canonical Clock Assurance inspector/control surface, including policy-neutral evidence inspection and explicit trusted-time refresh through the validated shared acquisition path; clarified that MethodMesh does not set Android system time.
- Added a discreet Home Time Assurance recency/evidence indicator that links to Workbench without inventing a universal freshness threshold.
- Made module and capability version/maturity universal metadata projected through shared UI, including compact version and Production/Development/Experimental badges.
- Required every independently callable capability to declare a nonblank implementation version and canonicalized legacy maturity labels to the typed Production/Development/Experimental vocabulary.
- Added FULL-envelope software provenance: module/capability ID, version and maturity are frozen when execution completes so later upgrades cannot rewrite historical sidecars; capability version is the authoritative reproducibility identifier.
- Raised the canonical FULL execution envelope schema to v2 for the software-provenance projection while retaining the independently versioned `time_assurance` schema.

## v1.24 - 2026-09-22

- Made temporal provenance universal in the canonical FULL execution envelope: every generated `methodmesh_full_json` now carries a versioned `time_assurance` object supplied centrally by shared Clock Assurance/output infrastructure.
- Separated universal policy-neutral clock evidence from capability/workflow `time_decision` records so ordinary capabilities do not inherit a hidden freshness/acceptance policy.
- Required explicit representation of wall-clock-only, reboot-broken, indeterminate and other reduced-assurance states rather than omission or silent promotion of device wall time.
- Made temporal sidecars historical execution evidence that later Sentinel/server evaluation may interpret but must not silently rewrite from newer anchors.
- Clarified ODK/XLSForm behaviour: canonical forms continue to capture `methodmesh_full_json`; they do not explode the complete clock object into synthetic universal flat leaves, although time-sensitive capabilities may declare convenient scalar temporal returns.

## v1.23 - 2026-09-22

- Established shared Clock Assurance as the project-wide basis for security-, validity-, expiry- and audit-sensitive current-time decisions.
- Separated Android wall-clock observation, same-boot monotonic elapsed time and externally validated trusted time anchors; made reboot a hard trust-continuity boundary.
- Defined trusted current time as bounded lower/upper intervals with acquisition-window, source-precision and oscillator-drift uncertainty rather than false point precision.
- Added independent clock-assurance and wall-clock anomaly states, including explicit rollback/divergence detection without discarding a still-valid monotonic trusted timeline.
- Required non-backed-up, Android-Keystore-authenticated anchor persistence with fail-closed corruption handling and safe recovery from fresh externally validated evidence.
- Tightened RFC 3161 clock-anchor admission: configured signer pinning must be bound to the token signer, certificate chain/time/use must validate, and unconfigured/custom TSAs do not silently establish Clock Assurance.
- Made capability-specific freshness/uncertainty policy and boundary-overlap handling normative, with evidence retained for ODK/Sentinel interpretation where time changes the decision.
...
- Made preset launch presentation a first-class contract with `AUTO`, `INTERACTIVE` and `BACKGROUND`; existing/imported presets without the field default safely to Auto.
- Advanced the protocol/preset library export schema to bundle v2 for first-class `launch_mode`; current imports remain backward-compatible with integrity-checked v1 bundles and migrate missing launch mode to Auto.
- Reframed preset authoring around separate Configuration, Run behaviour, After completion, Returned data and optional Logging sections; raw settings JSON is advanced detail rather than the primary UI.
- Retained `CORE`/`AUDIT`/`FULL` compatibility values while standardising user-facing returned-data language as Result / + Audit / + Full JSON.
- Clarified that `HOME` is the stored compatibility token for the user-facing Return completion behaviour and that Return means launch-origin closeout rather than unconditional navigation to MethodMesh Home.
- Made scheduled preset presentation follow the preset by default, with explicit Show UI / Background overrides; removed universal forced-headless scheduling as a valid design.
- Made **presets/protocols own what; scheduler owns when** normative and documented Once, Interval, Daily, Weekly, Monthly, Day sequence and advanced Cron schedule patterns.
- Distinguished elapsed interval anchors from local wall-clock calendar recurrence, including immediate-first-run and daylight-saving semantics.
- Documented exact-alarm permission/re-arming expectations, high-priority reminder behaviour and transient scheduled tasks that return to the app/desktop underneath after closeout.
