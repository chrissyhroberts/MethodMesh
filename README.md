...
- **Automatic barcode scanner** (`barcode.scan`) — captures QR, Data Matrix, PDF417, Aztec, and supported one-dimensional barcodes with automatic format detection. The exact decoded text is the primary result; safe HTTP(S) payloads also return an explicit link field. The deprecated `qr.scan` contract remains registered for existing forms during the compatibility window.
